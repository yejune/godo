...
description: >
  ...
  Do NOT use for refactoring existing code (use {{slot:BRAND}}-workflow-ddd instead)
...
metadata:
  ...
  context: "fork"
  agent: "manager-tdd"
  related-skills: "{{slot:BRAND}}-workflow-ddd, {{slot:BRAND}}-workflow-testing, {{slot:BRAND}}-foundation-quality"
...
- `development_mode: hybrid` + refactoring existing code → Use DDD instead ({{slot:BRAND}}-workflow-ddd)
