...
metadata:
  ...
  related-skills: "{{slot:BRAND}}-lang-kotlin, {{slot:BRAND}}-lang-flutter"
...
- `{{slot:BRAND}}-lang-kotlin` - Android counterpart for cross-platform projects
- `{{slot:BRAND}}-lang-flutter` - Flutter/Dart for cross-platform mobile
- `{{slot:BRAND}}-domain-backend` - API integration and backend communication
- `{{slot:BRAND}}-foundation-quality` - iOS security best practices
- `{{slot:BRAND}}-workflow-testing` - Xcode debugging and profiling
