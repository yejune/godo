...
- `{{slot:BRAND}}-lang-go` - Go systems programming patterns
- `{{slot:BRAND}}-domain-backend` - REST API architecture and microservices patterns
- `{{slot:BRAND}}-foundation-quality` - Security hardening for Rust applications
- `{{slot:BRAND}}-workflow-testing` - Test-driven development workflows
