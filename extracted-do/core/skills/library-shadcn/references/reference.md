...
- `{{slot:BRAND}}-domain-frontend` (Frontend development patterns)
- `{{slot:BRAND}}-baas-foundation` (Enterprise UI architecture)
- `{{slot:BRAND}}-security-api` (UI security implementation)
- `{{slot:BRAND}}-essentials-perf` (Performance optimization)
- `{{slot:BRAND}}-foundation-trust` (Accessibility and compliance)
- `{{slot:BRAND}}-domain-backend` (Backend integration)
- `{{slot:BRAND}}-baas-vercel-ext` (Frontend deployment)
- `{{slot:BRAND}}-domain-testing` (Component testing strategies)
