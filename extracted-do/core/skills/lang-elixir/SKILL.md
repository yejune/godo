...
- `{{slot:BRAND}}-domain-backend` - REST API and microservices architecture
- `{{slot:BRAND}}-domain-database` - SQL patterns and query optimization
- `{{slot:BRAND}}-workflow-testing` - DDD and testing strategies
- `{{slot:BRAND}}-essentials-debug` - AI-powered debugging
- `{{slot:BRAND}}-platform-deploy` - Deployment and infrastructure
