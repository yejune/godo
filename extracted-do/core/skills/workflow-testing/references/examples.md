  ...
  "auto_fixable": [
    {
      "file": "src/auth/service.py",
      "fix": "Add type hints",
      "apply_command": "{{slot:BRAND}}-workflow fix --file src/auth/service.py --type hints"
    }
  ]
}
```
...
```bash
# 전체 워크플로우 실행
{{slot:BRAND}}-workflow execute --project /project/src --mode full

# 개별 컴포넌트 실행
{{slot:BRAND}}-workflow debug --file app.py --error "AttributeError"
{{slot:BRAND}}-workflow refactor --directory src/ --max-risk medium
{{slot:BRAND}}-workflow profile --target function_name --types cpu,memory
{{slot:BRAND}}-workflow test --spec user_auth.spec --mode ddd
{{slot:BRAND}}-workflow review --project /project/src --trust-score-min 0.8

# CI 모드
{{slot:BRAND}}-workflow ci --commit abc123 --quality-gates strict
```
