...
- `{{slot:BRAND}}-domain-backend` - API design, database integration patterns
- `{{slot:BRAND}}-platform-deploy` - Azure, Docker, Kubernetes deployment
- `{{slot:BRAND}}-workflow-testing` - Testing strategies and patterns
- `{{slot:BRAND}}-foundation-quality` - Code quality standards
- `{{slot:BRAND}}-essentials-debug` - Debugging .NET applications
