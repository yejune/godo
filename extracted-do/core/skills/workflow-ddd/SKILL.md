...
description: >
  ...
  Do NOT use for writing new tests (use {{slot:BRAND}}-workflow-testing instead)
...
metadata:
  ...
  context: "fork"
  agent: "manager-ddd"
  related-skills: "{{slot:BRAND}}-tool-ast-grep, {{slot:BRAND}}-workflow-testing, {{slot:BRAND}}-foundation-quality"
---
...
- `development_mode: hybrid` + new package/module → Use TDD instead ({{slot:BRAND}}-workflow-tdd)
...
Load {{slot:BRAND}}-tool-ast-grep for detailed pattern syntax and rule creation.
