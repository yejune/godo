...
- [HARD] 하나의 작업 = 하나의 폴더 — 모든 산출물이 같은 디렉토리에 위치:
  - 분석: `.{{slot:BRAND_DIR}}/jobs/{YYMMDD}/{title-kebab-case}/analysis.md` (복잡한 작업만)
  - 아키텍처: `.{{slot:BRAND_DIR}}/jobs/{YYMMDD}/{title-kebab-case}/architecture.md` (복잡한 작업만)
  - 플랜: `.{{slot:BRAND_DIR}}/jobs/{YYMMDD}/{title-kebab-case}/plan.md`
  - 체크리스트: `.{{slot:BRAND_DIR}}/jobs/{YYMMDD}/{title-kebab-case}/checklist.md`
  - 완료 보고서: `.{{slot:BRAND_DIR}}/jobs/{YYMMDD}/{title-kebab-case}/report.md`
  - 서브 파일: `.{{slot:BRAND_DIR}}/jobs/{YYMMDD}/{title-kebab-case}/checklists/{order}_{agent-topic}.md`
- [HARD] 서브 파일의 `{order}`는 두 자리 숫자: `01`, `02`, ... `99`
- 디렉토리 없으면 자동 생성

### 예시 (복잡한 작업)
```
.{{slot:BRAND_DIR}}/jobs/260211/queue-library-migration/
...
### 예시 (단순한 작업)
```
.{{slot:BRAND_DIR}}/jobs/260211/login-api-security/
  ├── plan.md                        ← 플랜
  ├── checklist.md                   ← 메인 체크리스트
  ├── report.md                      ← 완료 보고서
  └── checklists/                    ← 에이전트별 서브
...
> 이 상태 기호는 `.{{slot:BRAND_DIR}}/jobs/` 체크리스트 시스템 전용이다.
...
복잡한 작업 시 `.{{slot:BRAND_DIR}}/jobs/{YYMMDD}/{title-kebab-case}/analysis.md`에 작성.
...
복잡한 작업 시 `.{{slot:BRAND_DIR}}/jobs/{YYMMDD}/{title-kebab-case}/architecture.md`에 작성.
