...
memory: user
skills: {{slot:BRAND}}-foundation-claude, {{slot:BRAND}}-workflow-project
---
...
- If `--moai` flag is present in the request, use `do-` prefix instead of `custom-`
...
Create SKILL.md and supporting files in `.claude/skills/<prefix>-<name>/` directory (prefix is `custom-` by default, or `do-` with `--moai` flag). Apply frontmatter, write content sections, and verify line count.
...
- With `--moai` flag: `do-<name>` → directory `.claude/skills/do-<name>/`

The `do-` namespace is reserved for MoAI-ADK system skills. Only use `do-` prefix when:
