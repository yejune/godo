...
Do의 철학(체크리스트, commit-as-proof, AI 안티패턴 7종, Real DB only)은 선언만으로 작동하지 않는다. 이 철학을 **실제로 강제하는 메커니즘**이 dev-* 규칙 파일들이다. 페르소나 패키지에서 이 규칙들을 빼면, 체크리스트 시스템이 상태 전이 규칙 없이 동작하고, 테스트 철학이 구체적 금지 조항 없이 선언만 남으며, 커밋 규율이 절차 없이 구호만 남는다.
