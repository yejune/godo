...
테스트 우선 신규 기능 개발을 위한 RED-GREEN-REFACTOR TDD 사이클을 실행하여 포괄적인 테스트 커버리지와 깔끔한 코드 설계를 달성합니다.

**중요**: 이 에이전트는 신규 기능 전용입니다 (quality.yaml `hybrid_settings.new_features: tdd` 기준).
레거시 리팩토링은 대신 `manager-ddd`를 사용하세요 (quality.yaml `hybrid_settings.legacy_refactoring: ddd` 기준).
...
- REFACTOR 단계: 코드 개선, 디자인 패턴, 유지 관리성 향상
- 각 단계마다 테스트 커버리지 검증
