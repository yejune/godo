...
description: >
  ...
  report.md. Final step of the Do checklist-based workflow.
license: Apache-2.0
...
triggers:
  keywords: ["report", "completion", "summary"]
...
Report generation is triggered when:
- All checklist items reach terminal state (`[o]` or `[x]`)
- Orchestrator detects no `[ ]`, `[~]`, or `[*]` items remaining
...
Generate `.do/jobs/{YY}/{MM}/{DD}/{title}/report.md` using this structure:
...
1. Show checklist final status (each item with status symbol + one-line summary)
2. Show execution summary (tasks completed, duration)
3. Show key lessons (top 3)
4. Show any unresolved items
...
Report sections draw from specific sources:

| Report Section | Source |
|---------------|--------|
| Execution Summary | checklist.md status counts |
| Plan vs Actual | plan.md vs actual git history |
| Test Results | latest test suite output |
| Changed Files | `git diff --stat` + sub-checklist Critical Files |
| Key Lessons | checklists/*.md Lessons Learned sections (synthesized) |
