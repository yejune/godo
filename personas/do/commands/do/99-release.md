---
description: "Do v2.x production release with agent delegation and quality validation. Target branch is always main. Tag format vX.Y.Z triggers GoReleaser. All git operations delegated to manager-git. Quality failures escalate to expert-debug."
...
- **Release URL**: https://github.com/do-focus/do-focus/releases/tag/vX.Y.Z
...
2. Edit `.do/config/sections/system.yaml` `do.version`
3. Edit `internal/template/templates/.do/config/sections/system.yaml` `do.version`
4. Commit: `git add pkg/version/version.go .do/config/sections/system.yaml internal/template/templates/.do/config/sections/system.yaml && git commit -m "chore: bump version to vX.Y.Z"`
...
4. Release URL: https://github.com/do-focus/do-focus/releases/tag/vX.Y.Z
```
...
- do-focus_X.Y.Z_windows_arm64.zip
- checksums.txt
...
\`\`\`bash
# Fresh install
curl -sSL https://raw.githubusercontent.com/do-focus/do-focus/main/install.sh | bash
do version
...
1. Uninstall Python version: `uv tool uninstall do-focus`
2. Install Go Edition (use commands above)
3. Update project templates: `do init`
...
1. Python 버전 제거: `uv tool uninstall do-focus`
2. Go 에디션 설치 (위 명령어 사용)
3. 프로젝트 템플릿 업데이트: `do init`
\`\`\`
RELEASE_EOF
)"
```
...
```
do-focus_X.Y.Z_darwin_arm64/
do-focus_X.Y.Z_darwin_arm64/do
do-focus_X.Y.Z_darwin_arm64/README.md
do-focus_X.Y.Z_darwin_arm64/LICENSE
```

**4. Verify archive naming matches checker logic:**

The `internal/update/checker.go` expects archives WITHOUT "v" prefix:
- Correct: `do-focus_2.1.0_darwin_arm64.tar.gz`
- Wrong: `do-focus_v2.1.0_darwin_arm64.tar.gz`

```bash
# Verify naming convention
gh release view vX.Y.Z --json assets --jq '.assets[].name' | grep -v "^do-focus_v"
```
...
- GitHub Release: https://github.com/do-focus/do-focus/releases/tag/vX.Y.Z
- GitHub Actions: https://github.com/do-focus/do-focus/actions
...
If local `.claude/` or `.do/` files are out of sync with the templates:
...
### Links
- GitHub Release: https://github.com/do-focus/do-focus/releases/tag/v2.1.0
...
# Expected: do-focus_2.1.0_darwin_arm64.tar.gz (without "v")
# If you see: do-focus_v2.1.0_darwin_arm64.tar.gz (with "v"), checker will fail
```
...
archiveName := fmt.Sprintf("do-focus_%s_%s_%s.%s", version, runtime.GOOS, runtime.GOARCH, ext)
```
...
```bash
$ git status
modified:   internal/cli/.claude/agents/do/manager-spec.md
modified:   internal/cli/.claude/commands/do/01-plan.md
...
- `.do/config/sections/system.yaml` shows 2.0.5
- `internal/template/templates/.do/config/sections/system.yaml` shows 2.0.5
...
grep -n "do.version" .do/config/sections/system.yaml
grep -n "do.version" internal/template/templates/.do/config/sections/system.yaml
