...
- [HARD] 에이전트의 작업은 다음 사이클을 반복하며, 몇 번을 실행해도 동일 결과를 보장해야 한다:
  ```
  1. READ:   서브 체크리스트 파일 읽기 → [o] 항목은 건너뛰기
  2. CLAIM:  작업할 항목을 [ ]→[~]로 갱신 + Progress Log에 시작 기록
  3. WORK:   코드 구현 (파일 수정/생성)
  4. VERIFY: 테스트/빌드 검증 → [~]→[*]로 갱신 + Progress Log에 검증 기록
  5. RECORD: 검증 통과 시 [*]→[o] + Acceptance Criteria 체크 + Lessons Learned 작성
  6. COMMIT: 코드 + 체크리스트를 함께 커밋 → Progress Log에 커밋 해시 기록
  ```
- [HARD] **READ 단계가 핵심**: 이미 [o]인 항목은 절대 재작업하지 않음 — 이것이 멱등성의 근거
- [HARD] **CLAIM 즉시 갱신**: 작업 시작과 동시에 체크리스트 파일을 [~]로 변경 — 중단 시 다음 에이전트가 상태를 알 수 있음
- [HARD] **COMMIT에 체크리스트 포함**: `git add`에 코드 파일 + 서브 체크리스트 파일을 함께 포함
...
### 체크리스트 미갱신 = VIOLATION [HARD]
- [HARD] 코드를 커밋했으나 서브 체크리스트가 초기 상태(`[ ]`)면 VIOLATION
- [HARD] 메인 체크리스트가 `[o]`인데 서브 체크리스트가 `[ ]`이면 불일치 VIOLATION
- [HARD] Progress Log가 비어있으면 VIOLATION — 최소 시작/완료 2개 항목 필수
- [HARD] Lessons Learned가 비어있는 `[o]` 완료는 VIOLATION

## 에이전트 중단 & 재개 (멱등 재개) [HARD]
...
- [HARD] 멱등 재개 보장: 새 에이전트가 READ 단계에서 기존 상태를 읽으므로 중복 작업 없음
- [HARD] `[~]` 상태 항목 발견 시: 해당 코드가 실제로 수정되었는지 확인 후, 수정됐으면 VERIFY부터, 아니면 WORK부터 재개

## 에이전트 리서치 위임 규칙 [HARD]
- [HARD] 파일 산출물이 필요한 리서치는 **general-purpose** 에이전트 사용 — Explore 에이전트는 Write 불가하므로 결과가 컨텍스트에 쌓여 폭발
- [HARD] Explore 에이전트는 코드 탐색/검색만 사용 — 파일 쓰기가 필요하면 general-purpose
- [HARD] 시스템 프롬프트에 이미 로드된 파일을 에이전트에게 **재읽기 시키지 않음** — 요약해서 전달
- [HARD] 에이전트에게 큰 파일(10K+ 문자) 여러 개를 동시에 읽게 시키지 않음 — 컨텍스트 초과 방지
- [HARD] Progressive Write: 에이전트는 리서치 결과를 발견 즉시 파일에 기록 — 마지막에 한꺼번에 쓰기 금지
- [HARD] 리서치 에이전트에게 산출물 파일 경로와 "발견할 때마다 파일에 추가 기록하라" 지시 필수
