...
description: >
  ...
  and Plan-Run-Sync integration for Do-ADK development methodology.
  Use when creating SPEC documents, writing EARS requirements, defining acceptance
  criteria, planning features, or orchestrating the /do plan phase.
  Do NOT use for implementation (use do-workflow-ddd instead)
...
metadata:
  ...
  updated: "2026-01-08"
  modularized: "true"
  tags: "workflow, spec, ears, requirements, do-adk, planning"
  author: "Do-ADK Team"
  context: "fork"
  agent: "Plan"
...
- Create new SPEC: /do:1-plan "user authentication system"
- Create parallel SPECs with Worktrees: /do:1-plan "login feature" "signup feature" --worktree
- Create SPEC with new branch: /do:1-plan "payment processing" --branch
- Update existing SPEC: /do:1-plan SPEC-001 "add OAuth support"
...
Constitution defines the project DNA that all SPECs must respect. Before creating any SPEC, verify alignment with project constitution defined in `.do/project/tech.md`.
...
- SPEC document creation in .do/specs/ directory
...
- do-workflow-testing skill reference for test patterns
...
- Command /do:1-plan "login feature" "signup feature" --worktree creates multiple SPECs
...
- .do/specs/SPEC-{ID}/: SPEC document directory containing 3 required files
...
- .do/memory/: Session state files (last-session-state.json)
- .do/docs/: Generated documentation (api-documentation.md)
...
- do-foundation-core: SPEC-First DDD methodology and TRUST 5 framework
- do-workflow-testing: DDD implementation and test automation
- do-workflow-project: Project initialization and configuration
- do-workflow-worktree: Git Worktree management for parallel development
...
- Step 1 PLAN: /do:1-plan "user authentication system"
- Step 2 RUN: /do:2-run SPEC-001
- Step 3 SYNC: /do:3-sync SPEC-001

Parallel Workflow:

- Create multiple SPECs: /do:1-plan "backend API" "frontend UI" "database schema" --worktree
- Session 1: /do:2-run SPEC-001 (backend API)
- Session 2: /do:2-run SPEC-002 (frontend UI)
- Session 3: /do:2-run SPEC-003 (database schema)
...
- SPEC document persists in .do/specs/ directory
- Session memory in .do/memory/ for cross-session context
...
### What Belongs in .do/specs/

The `.do/specs/` directory is EXCLUSIVELY for SPEC documents that define features to be implemented.
...
### What Does NOT Belong in .do/specs/

| Document Type         | Why Not SPEC                  | Correct Location                          |
| --------------------- | ----------------------------- | ----------------------------------------- |
| Security Audit        | Analyzes existing code        | `.do/reports/security-audit-{DATE}/`    |
| Performance Report    | Documents current metrics     | `.do/reports/performance-{DATE}/`       |
| Dependency Analysis   | Reviews existing dependencies | `.do/reports/dependency-review-{DATE}/` |
| Architecture Overview | Documents current state       | `.do/docs/architecture.md`              |
| API Reference         | Documents existing APIs       | `.do/docs/api-reference.md`             |
| Meeting Notes         | Records decisions made        | `.do/reports/meeting-{DATE}/`           |
| Retrospective         | Analyzes past work            | `.do/reports/retro-{DATE}/`             |

### Exclusion Rules

[HARD] Report vs SPEC Distinction:

Reports analyze what EXISTS → `.do/reports/`
SPECs define what will be BUILT → `.do/specs/`

[HARD] Documentation vs SPEC Distinction:

Documentation explains HOW TO USE → `.do/docs/`
SPECs define WHAT TO BUILD → `.do/specs/`
...
Problem: `.do/specs/SPEC-AUTH-001.md` exists as single file

Solution Steps:

1. Create directory: `mkdir -p .do/specs/SPEC-AUTH-001/`
2. Move content: `mv .do/specs/SPEC-AUTH-001.md .do/specs/SPEC-AUTH-001/spec.md`
...
find .do/specs -maxdepth 1 -name "SPEC-*.md" -type f
```
...
1. Find next available number:
   ```bash
   ls -d .do/specs/SPEC-*-[0-9][0-9][0-9] 2>/dev/null | sort -t- -k3 -n | tail -1
   ```
2. Assign number: `SPEC-REDESIGN` → `SPEC-REDESIGN-001`
3. Rename directory:
   ```bash
   mv .do/specs/SPEC-REDESIGN .do/specs/SPEC-REDESIGN-001
   ```
...
Problem: Analysis/audit document in `.do/specs/`
...
2. Create reports directory:
   ```bash
   mkdir -p .do/reports/security-audit-2025-01/
   ```
3. Move content:
   ```bash
   mv .do/specs/SPEC-SECURITY-AUDIT/* .do/reports/security-audit-2025-01/
   rmdir .do/specs/SPEC-SECURITY-AUDIT
   ```
...
1. Compare creation dates:
   ```bash
   ls -la .do/specs/ | grep SPEC-AUTH-001
   ```
2. Determine which is canonical (usually older one)
3. Renumber newer one to next available:
   ```bash
   mv .do/specs/SPEC-AUTH-001-duplicate .do/specs/SPEC-AUTH-002
   ```
...
ls -d .do/specs/SPEC-*/ 2>/dev/null | grep -v -E 'SPEC-[A-Z]+-[0-9]{3}'

# Check 4: Potential reports in specs
echo -e "\n[Check 4] Potential reports in specs (check manually):"
grep -l -r "findings\|recommendations\|audit\|analysis" .do/specs/*/spec.md 2>/dev/null
