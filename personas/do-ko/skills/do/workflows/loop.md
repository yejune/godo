...
- Marker types: `<do>DONE</do>`, `<do>COMPLETE</do>`
...
- If session duration exceeds 25 minutes OR iteration time doubling:
  - Save proactive checkpoint to $CLAUDE_PROJECT_DIR/.do/cache/loop-snapshots/memory-pressure.json
  - Warn user about memory pressure
  - Suggest resuming with /do:loop --resume memory-pressure
...
- Save iteration snapshot to $CLAUDE_PROJECT_DIR/.do/cache/loop-snapshots/
...
Snapshot location: $CLAUDE_PROJECT_DIR/.do/cache/loop-snapshots/
...
Loop state file: $CLAUDE_PROJECT_DIR/.do/cache/.do_loop_state.json
