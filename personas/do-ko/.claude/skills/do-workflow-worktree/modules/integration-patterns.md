...
Purpose: Overview of integration patterns for do-worktree with MoAI-ADK workflow, development tools, and external systems.
...
The basic integration workflow follows three phases. During the Plan Phase, the /moai:1-plan command auto-creates a worktree for the SPEC. During the Development Phase, use the do-worktree go command to navigate to the isolated environment and run /moai:2-run for DDD implementation. During the Sync Phase, use do-worktree sync to update the worktree and /moai:3-sync for documentation synchronization.
...
MoAI-ADK Integration: Refer to do-adk-integration.md
...
Detailed Reference: Refer to do-adk-integration.md for complete workflow patterns
...
For automatic SPEC worktree creation, integrate with /moai:1-plan by configuring auto-creation in worktree settings. Refer to do-adk-integration.md for implementation details.
...
For team collaboration with shared registries, configure team registry settings with developer-specific worktree prefixes. Refer to do-adk-integration.md for coordination patterns.

---

## Sub-Module References

MoAI-ADK Integration (do-adk-integration.md):
