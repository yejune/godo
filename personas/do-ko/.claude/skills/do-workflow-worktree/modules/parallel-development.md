...
To set up parallel development, create worktrees for each SPEC using do-worktree new with the SPEC ID and description. For parallel execution, navigate to each worktree using do-worktree go and run /moai:2-run in separate terminals or background processes. For integration, use do-worktree sync --all to synchronize all worktrees, then clean up merged worktrees with do-worktree clean --merged-only.
