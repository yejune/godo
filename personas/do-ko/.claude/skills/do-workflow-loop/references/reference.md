...
| `ralph.ast_grep.config_path`                        | string  | `.claude/skills/do-tool-ast-grep/rules/sgconfig.yml` | Path to AST-grep configuration                            |
...
Location: `.claude/skills/do-tool-ast-grep/rules/sgconfig.yml`
...
3. Check configuration file exists:
   ...
   ```bash
   ls .claude/skills/do-tool-ast-grep/rules/sgconfig.yml
   ```
    ...
    steps:
      ...
      - name: Commit fixes
        ...
        run: |
          ...
          git config user.email "ralph@do-adk.dev"
          git add .
