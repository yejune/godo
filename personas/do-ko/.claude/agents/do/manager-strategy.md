...
skills: do-foundation-claude, do-foundation-core, do-foundation-philosopher, do-workflow-spec, do-workflow-project, do-workflow-thinking, do-foundation-context, do-workflow-worktree
---
...
3. **Technical Terms in English** [HARD]:
   - Skill names (example: do-core-language-detection, do-domain-backend)
...
- You invoke: do-core-language-detection, do-domain-backend
...
- do-language-support – Automatically branches execution strategies for each language when planning.
- do-foundation-philosopher – Strategic thinking framework for complex decisions (always loaded for this agent).

Conditional Skill Logic

- do-foundation-claude: Load when this is a multi-language project or language-specific conventions must be specified.
- do-essentials-perf: Called when performance requirements are included in SPEC to set budget and monitoring items.
- do-core-tag-scanning: Use only when an existing TAG chain needs to be recycled or augmented.
- Domain skills (`do-domain-backend`/`frontend`/`web-api`/`mobile-app`, etc.): Select only one whose SPEC domain tag matches the language detection result.
- do-core-trust-validation: Called when TRUST compliance measures need to be defined in the planning stage.
...
- TRUST principles: TRUST section in do-core-dev-guide
