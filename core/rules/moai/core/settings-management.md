...
`.{{slot:BRAND_DIR}}/config/` - MoAI-specific settings:
...
```json
{
  "statusLine": {
    "type": "command",
    "command": ".{{slot:BRAND_DIR}}/status_line.sh"
  }
}
```
...
Team behavior is controlled by the `workflow.team` section in `.{{slot:BRAND_DIR}}/config/sections/workflow.yaml`:
...
- Skill("{{slot:BRAND}}-workflow-project") for project setup
- Skill("{{slot:BRAND}}-foundation-core") for quality framework
