...
skills: {{slot:BRAND}}-foundation-claude, {{slot:BRAND}}-foundation-core, {{slot:BRAND}}-docs-generation, {{slot:BRAND}}-workflow-jit-docs, {{slot:BRAND}}-workflow-templates, {{slot:BRAND}}-library-mermaid, {{slot:BRAND}}-library-nextra, {{slot:BRAND}}-formats-data, {{slot:BRAND}}-foundation-context
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
...
checkpoint_strategy:
  ...
  location: $CLAUDE_PROJECT_DIR/.{{slot:BRAND_DIR}}/memory/checkpoints/docs/
...
- Checkpoint location: `.{{slot:BRAND_DIR}}/memory/checkpoints/docs/`
...
/{{slot:BRAND_CMD}} sync SPEC-AUTH-001

# Resume from checkpoint after crash
/{{slot:BRAND_CMD}} sync SPEC-AUTH-001 --resume latest
```
...
- {{slot:BRAND}}-foundation-core: SPEC-first DDD, TRUST 5 framework, execution rules
- {{slot:BRAND}}-workflow-docs: Documentation workflow, validation scripts
- {{slot:BRAND}}-library-mermaid: Mermaid diagram creation and validation
- {{slot:BRAND}}-foundation-claude: Claude Code authoring patterns, skills/agents/commands
- {{slot:BRAND}}-library-nextra: Nextra framework setup and optimization

# Conditional skills (auto-loaded by MoAI when needed)

conditional_skills = [
"{{slot:BRAND}}-domain-uiux", # WCAG compliance, accessibility patterns, Pencil MCP integration
"{{slot:BRAND}}-lang-python", # Python documentation patterns
"{{slot:BRAND}}-lang-typescript", # TypeScript documentation patterns
"{{slot:BRAND}}-workflow-project", # Project documentation management
"{{slot:BRAND}}-ai-nano-banana" # AI content generation
]
...
- **Documentation Synchronization**: Sync Nextra docs with .{{slot:BRAND_DIR}}/docs/ directory structure
...
3. **Documentation Synchronization**:
   ...
   - Target: .{{slot:BRAND_DIR}}/docs/ directory
