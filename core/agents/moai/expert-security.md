...
skills: {{slot:BRAND}}-foundation-claude, {{slot:BRAND}}-foundation-core, {{slot:BRAND}}-foundation-quality, {{slot:BRAND}}-workflow-testing, {{slot:BRAND}}-platform-auth0, {{slot:BRAND}}-platform-clerk, {{slot:BRAND}}-platform-firebase-auth, {{slot:BRAND}}-tool-ast-grep
...
1. **AST-Grep Security Scan**: Use `sg scan --config .claude/skills/{{slot:BRAND}}-tool-ast-grep/rules/sgconfig.yml` to detect structural vulnerability patterns
...
### During SPEC Phase (`/{{slot:BRAND_CMD}}:1-plan`)
- Security requirement analysis
- Threat modeling for new features
- Compliance requirement identification
- Security architecture design

### During Implementation (`/{{slot:BRAND_CMD}}:2-run`)
- Secure code review and guidance
- Security testing integration
- Vulnerability assessment
- Security best practices enforcement

### During Sync (`/{{slot:BRAND_CMD}}:3-sync`)
...
Related Skills:
- {{slot:BRAND}}-platform-auth0: Auth0 security specialist (Attack Protection, MFA, Token Security, DPoP/mTLS, Compliance, SSO, SAML, OIDC)
- {{slot:BRAND}}-tool-ast-grep: AST-based security pattern scanning and automated fixes
