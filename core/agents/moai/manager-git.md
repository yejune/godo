...
skills: {{slot:BRAND}}-foundation-claude, {{slot:BRAND}}-foundation-core, {{slot:BRAND}}-workflow-project, {{slot:BRAND}}-workflow-worktree, {{slot:BRAND}}-workflow-testing, {{slot:BRAND}}-foundation-quality
---
...
Key Advantage: Simple, consistent GitHub Flow for all modes. Users select mode manually via `.{{slot:BRAND_DIR}}/config.json` without auto-switching.

This is a dedicated agent that optimizes and processes all Git operations in {{slot:BRAND}}-adk-go for each mode.
...
- Commit messages: Read git_commit_messages from .{{slot:BRAND_DIR}}/config/sections/language.yaml
...
- {{slot:BRAND}}-foundation-claude – Provides Claude Code agent patterns, hook integration, settings management
- {{slot:BRAND}}-workflow-project – Provides Git workflow strategies, GitHub Flow patterns, project configuration
- {{slot:BRAND}}-foundation-quality – Provides Git command patterns, validation scripts, error handling
...
2. Load {{slot:BRAND}}-workflow-project skill for branch strategy
...
- Ensure all Git tasks follow TRUST principles from {{slot:BRAND}}-core-dev-guide
...
- Manually enable via `.{{slot:BRAND_DIR}}/config/config.yaml` configuration
...
File Location: `.{{slot:BRAND_DIR}}/config/config.yaml`
...
1. When writing a SPEC (`/{{slot:BRAND_CMD}}:1-plan`):
...
2. When implementing DDD (`/{{slot:BRAND_CMD}}:2-run`):
...
3. When synchronization completes (`/{{slot:BRAND_CMD}}:3-sync`):
...
- Read git_commit_messages from .{{slot:BRAND_DIR}}/config/sections/language.yaml
...
- Access: `.{{slot:BRAND_DIR}}/config/sections/language.yaml`
...
- Read base branch from `.{{slot:BRAND_DIR}}/config/config.yaml`
...
- Read configuration from `.{{slot:BRAND_DIR}}/config/config.yaml`
...
- Signal readiness for next /{{slot:BRAND_CMD}}:1-plan
...
This agent participates in the /{{slot:BRAND_CMD}}:2-run Phase 3 chain. Context must be properly received to execute appropriate Git operations.
...
**Output Context** (returned to /{{slot:BRAND_CMD}}:2-run command):
...
This section defines how manager-git handles the `auto_branch` configuration setting from `.{{slot:BRAND_DIR}}/config/sections/git-strategy.yaml`.

### Configuration Reading

Before any branch operation, read the auto_branch setting:

1. Locate configuration file: `.{{slot:BRAND_DIR}}/config/sections/git-strategy.yaml`
...
- Recommendation: Suggest running /{{slot:BRAND_CMD}}:0-project to initialize config
