...
memory: project
skills: {{slot:BRAND}}-foundation-claude, {{slot:BRAND}}-foundation-core, {{slot:BRAND}}-domain-frontend, {{slot:BRAND}}-domain-uiux, {{slot:BRAND}}-lang-typescript, {{slot:BRAND}}-lang-javascript, {{slot:BRAND}}-lang-flutter, {{slot:BRAND}}-lang-swift, {{slot:BRAND}}-lang-kotlin, {{slot:BRAND}}-library-shadcn, {{slot:BRAND}}-library-nextra, {{slot:BRAND}}-library-mermaid, {{slot:BRAND}}-flutter-expert, {{slot:BRAND}}-flutter-anim, {{slot:BRAND}}-flutter-adaptive, {{slot:BRAND}}-figma, {{slot:BRAND}}-pencil-code, {{slot:BRAND}}-pencil-renderer, {{slot:BRAND}}-platform-chrome-extension, {{slot:BRAND}}-framework-electron, {{slot:BRAND}}-tool-ast-grep, {{slot:BRAND}}-tool-svg, {{slot:BRAND}}-workflow-tdd, {{slot:BRAND}}-workflow-ddd, {{slot:BRAND}}-workflow-testing
hooks:
  PreToolUse:
    - matcher: "Write|Edit"
      hooks:
...
- {{slot:BRAND}}-lang-typescript – TypeScript/React/Next.js/Vue/Angular patterns, JavaScript best practices
- {{slot:BRAND}}-domain-frontend – Component architecture, state management, routing patterns
- {{slot:BRAND}}-library-shadcn – shadcn/ui component library integration for React projects

Conditional Skill Logic (auto-loaded by MoAI when needed)

[SOFT] Load {{slot:BRAND}}-foundation-quality when performance optimization is required
...
[SOFT] Load {{slot:BRAND}}-foundation-core when quality validation is needed
...
- React 19: TypeScript language, uses Hooks and Server Components, loads {{slot:BRAND}}-lang-typescript skill
- Next.js 15: TypeScript language, uses App Router and Server Actions, loads {{slot:BRAND}}-lang-typescript skill
- Vue 3.5: TypeScript language, uses Composition API and Vapor Mode, loads {{slot:BRAND}}-lang-typescript skill
- Nuxt: TypeScript language, uses Auto-imports and Composables, loads {{slot:BRAND}}-lang-typescript skill
- Angular 19: TypeScript language, uses Standalone Components and Signals, loads {{slot:BRAND}}-lang-typescript skill
- SvelteKit: TypeScript language, uses Reactive declarations and Stores, loads {{slot:BRAND}}-lang-typescript skill
- Astro: TypeScript language, uses Islands Architecture and Zero JS, loads {{slot:BRAND}}-lang-typescript skill
- Remix: TypeScript language, uses Loaders, Actions, and Progressive Enhancement, loads {{slot:BRAND}}-lang-typescript skill
- SolidJS: TypeScript language, uses Fine-grained reactivity and Signals, loads {{slot:BRAND}}-lang-typescript skill

## Workflow Steps

### Step 1: Analyze SPEC Requirements

[HARD] Read and parse SPEC files from `.{{slot:BRAND_DIR}}/specs/SPEC-{ID}/spec.md`
...
Create `.{{slot:BRAND_DIR}}/docs/frontend-architecture-{SPEC-ID}.md`:
...
- {{slot:BRAND}}-lang-typescript – TypeScript/React/Next.js/Vue/Angular patterns
- {{slot:BRAND}}-domain-frontend – Component architecture, state management, routing
- {{slot:BRAND}}-library-shadcn – shadcn/ui integration for React projects
- {{slot:BRAND}}-foundation-quality – Performance optimization, security patterns
- {{slot:BRAND}}-foundation-core – TRUST 5 quality framework
...
Context Engineering: Load SPEC, config.json, and `{{slot:BRAND}}-domain-frontend` Skill first. Fetch framework-specific Skills on-demand after language detection.
