...
skills: {{slot:BRAND}}-foundation-claude, {{slot:BRAND}}-foundation-core, {{slot:BRAND}}-foundation-philosopher, {{slot:BRAND}}-workflow-spec, {{slot:BRAND}}-workflow-project, {{slot:BRAND}}-workflow-thinking, {{slot:BRAND}}-foundation-context, {{slot:BRAND}}-workflow-worktree
---
...
3. **Technical Terms in English** [HARD]:
   - Skill names (example: {{slot:BRAND}}-core-language-detection, {{slot:BRAND}}-domain-backend)
...
- You invoke: {{slot:BRAND}}-core-language-detection, {{slot:BRAND}}-domain-backend
...
- {{slot:BRAND}}-language-support – Automatically branches execution strategies for each language when planning.
- {{slot:BRAND}}-foundation-philosopher – Strategic thinking framework for complex decisions (always loaded for this agent).

Conditional Skill Logic

- {{slot:BRAND}}-foundation-claude: Load when this is a multi-language project or language-specific conventions must be specified.
- {{slot:BRAND}}-essentials-perf: Called when performance requirements are included in SPEC to set budget and monitoring items.
- {{slot:BRAND}}-core-tag-scanning: Use only when an existing TAG chain needs to be recycled or augmented.
- Domain skills (`{{slot:BRAND}}-domain-backend`/`frontend`/`web-api`/`mobile-app`, etc.): Select only one whose SPEC domain tag matches the language detection result.
- {{slot:BRAND}}-core-trust-validation: Called when TRUST compliance measures need to be defined in the planning stage.
...
- **Read SPEC Directory Structure** [HARD]:
  - Each SPEC is a **folder** (e.g., `.{{slot:BRAND_DIR}}/specs/SPEC-001/`)
...
1. Locate the SPEC folder in `.{{slot:BRAND_DIR}}/specs/SPEC-{ID}/` directory
...
- For SPEC-001: Read `.{{slot:BRAND_DIR}}/specs/SPEC-001/spec.md`, `.{{slot:BRAND_DIR}}/specs/SPEC-001/plan.md`, `.{{slot:BRAND_DIR}}/specs/SPEC-001/acceptance.md`
...
- manager-spec: Create SPEC file (`.{{slot:BRAND_DIR}}/specs/`)
...
This agent participates in the /{{slot:BRAND_CMD}}:2-run Phase chain. Context must be properly received and passed to maintain workflow continuity.

**Input Context** (from /{{slot:BRAND_CMD}}:2-run command):
...
- **SPEC Directory Structure**:
  - Location: `.{{slot:BRAND_DIR}}/specs/SPEC-{ID}/`
  - Files: `spec.md`, `plan.md`, `acceptance.md`
  - Example: `.{{slot:BRAND_DIR}}/specs/SPEC-001/spec.md`
- Development guide: {{slot:BRAND}}-core-dev-guide
- TRUST principles: TRUST section in {{slot:BRAND}}-core-dev-guide
- TAG Guide: TAG Chain section in {{slot:BRAND}}-core-dev-guide
