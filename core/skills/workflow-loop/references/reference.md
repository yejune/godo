...
| `ralph.ast_grep.config_path`                        | string  | `.claude/skills/{{slot:BRAND}}-tool-ast-grep/rules/sgconfig.yml` | Path to AST-grep configuration                            |
...
Location: `.{{slot:BRAND_DIR}}/cache/.moai_loop_state.json`
...
Location: `.claude/skills/{{slot:BRAND}}-tool-ast-grep/rules/sgconfig.yml`
...
- `/{{slot:BRAND_CMD}}:loop` command does nothing
...
1. Check if Ralph is enabled:
   ...
   ```yaml
   # .{{slot:BRAND_DIR}}/config/sections/ralph.yaml
   ralph:
     enabled: true
   ```
...
4. Verify state file is writable:
   ```bash
   mkdir -p .{{slot:BRAND_DIR}}/cache
   chmod 755 .{{slot:BRAND_DIR}}/cache
   ```
...
3. Check configuration file exists:
   ...
   ```bash
   ls .claude/skills/{{slot:BRAND}}-tool-ast-grep/rules/sgconfig.yml
   ```
    ...
    steps:
      ...
      - name: Commit fixes
        ...
        run: |
          ...
          git config user.email "ralph@{{slot:BRAND}}-adk.dev"
          git add .
