...
/{{slot:BRAND_CMD}}:loop --max-iterations 5

# Start with specific completion criteria
/{{slot:BRAND_CMD}}:loop --zero-warnings true
```

**Loop State Management:**

```json
// .{{slot:BRAND_DIR}}/cache/.moai_loop_state.json
{
  "active": true,
  "iteration": 1,
  "max_iterations": 10,
...
/{{slot:BRAND_CMD}}:fix src/auth.py

# Fix with specific severity threshold
/{{slot:BRAND_CMD}}:fix --severity error
```

**Example Session:**

```
User: /{{slot:BRAND_CMD}}:fix
...
ralph:
  ...
  ast_grep:
    ...
    quality_scan: true
    custom_rules:
      - .{{slot:BRAND_DIR}}/ast-grep/security/**/*.yml
      - .{{slot:BRAND_DIR}}/ast-grep/quality/**/*.yml
...
ralph:
  ...
  ast_grep:
    enabled: true
    rules:
      - .{{slot:BRAND_DIR}}/ast-grep/python/**/*.yml
...
ralph:
  ...
  ast_grep:
    enabled: true
    rules:
      - .{{slot:BRAND_DIR}}/ast-grep/react/**/*.yml
      - .{{slot:BRAND_DIR}}/ast-grep/typescript/**/*.yml
```
...
def load_loop_state() -> dict:
    state_file = Path(".{{slot:BRAND_DIR}}/cache/.moai_loop_state.json")
...
```yaml
# .{{slot:BRAND_DIR}}/ast-grep/security/sql-injection.yml
...
```yaml
# .{{slot:BRAND_DIR}}/ast-grep/security/xss-prevention.yml
id: xss-react
language: typescript
rule:
...
```yaml
# .{{slot:BRAND_DIR}}/ast-grep/security/hardcoded-secrets.yml
...
jobs:
  ralph-check:
    ...
    steps:
      ...
      - name: Run Ralph Loop
        run: |
          claude -p "/{{slot:BRAND_CMD}}:loop --max-iterations 5" \
      ...
      - name: Check Results
        run: |
          python .{{slot:BRAND_DIR}}/scripts/check_ralph_results.py
```
    ...
    steps:
      ...
      - name: Run ${{ matrix.check }} Check
        run: |
          case "${{ matrix.check }}" in
            lsp)
              claude -p "/{{slot:BRAND_CMD}}:fix --severity error" \
      ...
      - name: Upload Results
        ...
        with:
          ...
          path: .{{slot:BRAND_DIR}}/reports/${{ matrix.check }}
```
...
if [ $? -ne 0 ]; then
    echo "❌ LSP errors found. Run '/{{slot:BRAND_CMD}}:fix' to resolve."
    exit 1
fi
...
# Install MoAI-ADK
RUN go install github.com/modu-ai/{{slot:BRAND}}-adk-go/cmd/moai@latest
...
RUN claude -p "/{{slot:BRAND_CMD}}:loop --max-iterations 3" \
    --allowedTools "Read,Write,Edit" \
...
```bash
# 1. Start development
/{{slot:BRAND_CMD}}:1-plan "User authentication system"
...
# 3. Verify quality
/{{slot:BRAND_CMD}}:fix --severity warning

# 4. Run security scan
moai ast-grep scan --security

# 5. Sync documentation
/{{slot:BRAND_CMD}}:3-sync
```
...
```bash
# View LSP logs
cat .{{slot:BRAND_DIR}}/logs/lsp_diagnostic.log
...
```bash
# View current state
cat .{{slot:BRAND_DIR}}/cache/.moai_loop_state.json

# Reset loop state
rm .{{slot:BRAND_DIR}}/cache/.moai_loop_state.json

# View loop logs
cat .{{slot:BRAND_DIR}}/logs/loop_controller.log
```
