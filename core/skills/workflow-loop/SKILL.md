...
- /{{slot:BRAND_CMD}}: One-click Plan-Run-Sync automation (default)
- /{{slot:BRAND_CMD}} loop: Start feedback loop
- /{{slot:BRAND_CMD}} fix: One-time auto-fix
...
The Ralph Engine follows a layered architecture. User commands such as /{{slot:BRAND_CMD}}:loop, /{{slot:BRAND_CMD}}:fix, and /{{slot:BRAND_CMD}} enter the Command Layer. The Command Layer invokes the Hook System, which contains the PostToolUse Hook for LSP diagnostics and the Stop Hook for loop control. The Hook System connects to Backend Services including the LSP Client (MoAILSPClient), AST-grep Scanner, and Test Runner. Backend Services feed into Completion Check which evaluates whether errors are zero, tests pass, and coverage is met. Based on the Completion Check result, the system either continues the loop or completes.
...
The Stop hook is triggered after each Claude response. The hook reads the loop state file located at .{{slot:BRAND_DIR}}/cache/.moai_loop_state.json. This state contains active status (true or false), current iteration number, max_iterations limit, last_error_count from previous iteration, and completion_reason when finished.
...
For GitHub Actions integration, create a workflow step that runs Claude with the /{{slot:BRAND_CMD}}:loop command and max-iterations flag. Set the MOAI_LOOP_ACTIVE environment variable to true to enable loop mode.
...
Review the max_iterations setting to ensure it allows sufficient iterations. Review completion conditions to verify they are achievable. Send any message to interrupt the loop, or delete the state file (.{{slot:BRAND_DIR}}/cache/.moai_loop_state.json) to reset.

## Works Well With

Skills:

- {{slot:BRAND}}-foundation-quality: TRUST 5 validation
- {{slot:BRAND}}-tool-ast-grep: Security scanning patterns
- {{slot:BRAND}}-workflow-testing: DDD integration
- {{slot:BRAND}}-lang-python: Python-specific patterns
- {{slot:BRAND}}-lang-typescript: TypeScript patterns
...
- /{{slot:BRAND_CMD}}:3-sync: Documentation sync
...
Configuration is stored at .{{slot:BRAND_DIR}}/config/sections/ralph.yaml.

Loop state is stored at .{{slot:BRAND_DIR}}/cache/.moai_loop_state.json.
...
Skill Name: {{slot:BRAND}}-workflow-loop (formerly {{slot:BRAND}}-ralph)
