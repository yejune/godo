# {{slot:BRAND}}-foundation-context Reference
...
- After `/{{slot:BRAND_CMD}}:1-plan` completion
...
Workflow Sequence:
1. `/{{slot:BRAND_CMD}}:1-plan` execution
2. `/clear` (mandatory - saves 45-50K tokens)
3. `/{{slot:BRAND_CMD}}:2-run SPEC-XXX`
4. Multi-agent handoffs
5. `/{{slot:BRAND_CMD}}:3-sync SPEC-XXX`
...
- `{{slot:BRAND}}-foundation-claude`: Claude Code authoring and configuration
- `{{slot:BRAND}}-foundation-core`: Core execution patterns and SPEC workflow
- `{{slot:BRAND}}-workflow-project`: Project management and documentation
- `{{slot:BRAND}}-cc-memory`: Memory management and persistence
