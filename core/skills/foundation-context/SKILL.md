---
name: {{slot:BRAND}}-foundation-context
description: >
  ...
  (use {{slot:BRAND}}-foundation-core instead).
license: Apache-2.0
...
metadata:
  ...
  aliases: "{{slot:BRAND}}-foundation-context"
  replaces: "{{slot:BRAND}}-core-context-budget, {{slot:BRAND}}-core-session-state"
...
Mandatory /clear Points: After /{{slot:BRAND_CMD}}:1-plan completion to save 45-50K tokens. When context exceeds 150K tokens to prevent overflow. When conversation exceeds 50 messages to remove stale history. Before major phase transitions for clean slate. During model switches for Haiku to Sonnet handoffs.
...
- {{slot:BRAND}}-cc-memory - Memory management and context persistence
- {{slot:BRAND}}-cc-configuration - Session configuration and preferences
- {{slot:BRAND}}-core-workflow - Workflow state persistence and recovery
- {{slot:BRAND}}-cc-agents - Agent state management across sessions
- {{slot:BRAND}}-foundation-trust - Quality gate integration
...
SPEC-First Workflow: Execute /{{slot:BRAND_CMD}}:1-plan, then mandatory /clear to save 45-50K tokens, then /{{slot:BRAND_CMD}}:2-run SPEC-XXX, then multi-agent handoffs with Pattern 4, then /{{slot:BRAND_CMD}}:3-sync SPEC-XXX, then session state persistence with Pattern 3.
