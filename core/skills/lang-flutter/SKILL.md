...
metadata:
  ...
  related-skills: "{{slot:BRAND}}-lang-swift, {{slot:BRAND}}-lang-kotlin"
...
- `{{slot:BRAND}}-lang-swift` - iOS native integration for platform channels
- `{{slot:BRAND}}-lang-kotlin` - Android native integration for platform channels
- `{{slot:BRAND}}-domain-backend` - API integration and backend communication
- `{{slot:BRAND}}-quality-security` - Mobile security best practices
- `{{slot:BRAND}}-essentials-debug` - Flutter debugging and DevTools
