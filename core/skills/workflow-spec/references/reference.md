...
User Request
    ↓
/{{slot:BRAND_CMD}}:1-plan "feature description"
    ↓
...
User Request
    ↓
/{{slot:BRAND_CMD}}:1-plan "feature1" "feature2" "feature3" --worktree
    ↓
...
│ /{{slot:BRAND_CMD}}:2-run │ /{{slot:BRAND_CMD}}:2-run │ /{{slot:BRAND_CMD}}:2-run │
└─────────────┴─────────────┴─────────────┘
    ↓
Worktree merge to main branch
    ↓
/{{slot:BRAND_CMD}}:3-sync SPEC-001 SPEC-002 SPEC-003
    ↓
...
```
/{{slot:BRAND_CMD}}:1-plan "database schema" --branch
    ↓
...
Database schema implemented
    ↓
/{{slot:BRAND_CMD}}:1-plan "backend API" --branch
    ↓
...
Backend API implemented
    ↓
/{{slot:BRAND_CMD}}:1-plan "frontend UI" --branch
    ↓
SPEC-003 created (depends on SPEC-002)
    ↓
/{{slot:BRAND_CMD}}:2-run SPEC-003
    ↓
