...
metadata:
  ...
  related-skills: "{{slot:BRAND}}-lang-java, {{slot:BRAND}}-lang-swift"
...
- `{{slot:BRAND}}-lang-java` - Java interoperability and Spring Boot patterns
- `{{slot:BRAND}}-domain-backend` - REST API, GraphQL, microservices architecture
- `{{slot:BRAND}}-domain-database` - JPA, Exposed, R2DBC patterns
- `{{slot:BRAND}}-quality-testing` - JUnit 5, MockK, TestContainers integration
- `{{slot:BRAND}}-infra-docker` - JVM container optimization
