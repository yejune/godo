...
```bash
# Phase 1: SPEC 생성
/{{slot:BRAND_CMD}}:1-plan "JWT 기반 사용자 인증 시스템 구현"
...
**Output**:
```markdown
Phase 1 Result (.{{slot:BRAND_DIR}}/specs/SPEC-001/spec.md):
==============================================
ID: SPEC-001
...
```bash
# 잘못된 예시
/{{slot:BRAND_CMD}}:1-plan "feature"  # 30K 사용
/{{slot:BRAND_CMD}}:2-run SPEC-001    # 이전 컨텍스트 유지 → 토큰 부족!
```
...
/{{slot:BRAND_CMD}}:2-run SPEC-001    # 새로운 180K 예산으로 시작
```
