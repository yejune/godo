# {{slot:BRAND}}-foundation-core Reference
...
{{slot:BRAND}}-trust check --all --verbose

# Token budget analysis
{{slot:BRAND}}-context analyze --show-breakdown

# Agent routing debug
{{slot:BRAND}}-agent route --task "description" --dry-run

# SPEC validation
{{slot:BRAND}}-spec validate SPEC-001 --format EARS
```
...
- TRUST 5 logs: `.{{slot:BRAND_DIR}}/logs/trust5/`
- SPEC artifacts: `.{{slot:BRAND_DIR}}/specs/`
- Agent execution logs: `.{{slot:BRAND_DIR}}/logs/agents/`
- Token usage history: `.{{slot:BRAND_DIR}}/logs/tokens/`
...
- {{slot:BRAND}}-workflow-project - Project management with core principles
- {{slot:BRAND}}-workflow-testing - Testing workflows with TRUST 5
- {{slot:BRAND}}-workflow-templates - Template management integration
