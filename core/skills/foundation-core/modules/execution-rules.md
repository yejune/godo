...
| `Skill()` | Knowledge invocation | `Skill("{{slot:BRAND}}-foundation-core")` |
...
1. When running `/{{slot:BRAND_CMD}}:1-plan`, user prompted: "Create branch?"
...
2. Running `/{{slot:BRAND_CMD}}:2-run`: DDD commits + auto push
3. Running `/{{slot:BRAND_CMD}}:3-sync`: Doc commits + suggest PR creation (user choice)
...
1. When running `/{{slot:BRAND_CMD}}:1-plan`, user prompted once: "Enable automatic branch creation?"
...
2. Running `/{{slot:BRAND_CMD}}:2-run`: DDD commits + auto push (to feature branch)
3. Running `/{{slot:BRAND_CMD}}:3-sync`: Doc commits + prepare PR
...
MoAI's Behavior (prompt_always=false, auto_enabled=false):
1. When running `/{{slot:BRAND_CMD}}:1-plan`, user prompted once: "Enable automatic branch creation and Draft PR creation?"
...
Execute `/clear` after `/{{slot:BRAND_CMD}}:1-plan` completion in ALL modes.
...
1. Immediately after `/{{slot:BRAND_CMD}}:1-plan` (mandatory)
...
- Create documentation in `.{{slot:BRAND_DIR}}/` directory
