...
1. SPEC Generation - EARS format requirements (/{{slot:BRAND_CMD}}:1-plan)
2. Domain-Driven Development - ANALYZE-PRESERVE-IMPROVE (/{{slot:BRAND_CMD}}:2-run)
3. Documentation Sync - Auto-generated docs (/{{slot:BRAND_CMD}}:3-sync)

Three-Phase Workflow:
```
Phase 1: SPEC → spec-builder → .{{slot:BRAND_DIR}}/specs/SPEC-XXX/spec.md
...
Skills:
- {{slot:BRAND}}-workflow-testing - Test frameworks

Commands:
- /{{slot:BRAND_CMD}}:1-plan - SPEC generation (Phase 1)
- /{{slot:BRAND_CMD}}:2-run - DDD implementation (Phase 2)
- /{{slot:BRAND_CMD}}:3-sync - Documentation sync (Phase 3)
