...
Purpose: Extended documentation modules for {{slot:BRAND}}-foundation-core skill, providing deep dives into each foundational principle.
...
Quick Access:
- SPEC generation (/{{slot:BRAND_CMD}}:1-plan)
- EARS format patterns
- DDD implementation (/{{slot:BRAND_CMD}}:2-run)
- Documentation sync (/{{slot:BRAND_CMD}}:3-sync)
...
Quick Access:
- /{{slot:BRAND_CMD}}:0-project (Project init)
- /{{slot:BRAND_CMD}}:1-plan (SPEC generation)
- /{{slot:BRAND_CMD}}:2-run (DDD implementation)
- /{{slot:BRAND_CMD}}:3-sync (Documentation)
- /{{slot:BRAND_CMD}}:9-feedback (Improvement)
- /{{slot:BRAND_CMD}}:99-release (Deployment)
...
skill_path = Path(".claude/skills/{{slot:BRAND}}-foundation-core")
...
class ModuleSearch:
 ...
 modules_dir = Path(".claude/skills/{{slot:BRAND}}-foundation-core/modules")
 results = []
...
Skills:
- {{slot:BRAND}}-foundation-core (parent skill)
- {{slot:BRAND}}-cc-skill-factory (skill creation)
- {{slot:BRAND}}-core-agent-factory (agent creation)
...
Commands:
- /{{slot:BRAND_CMD}}:1-plan (SPEC-First DDD)
- /{{slot:BRAND_CMD}}:2-run (DDD implementation)
- /{{slot:BRAND_CMD}}:3-sync (Documentation)
