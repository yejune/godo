...
- /{{slot:BRAND_CMD}}:2-run → Enforces ≥85% coverage
...
```bash
# 1. Generate SPEC
/{{slot:BRAND_CMD}}:1-plan "Implement user authentication with JWT tokens"

# 2. spec-builder creates:
.{{slot:BRAND_DIR}}/specs/SPEC-001/
...
# Validate skill
result = validator.validate_skill(".claude/skills/{{slot:BRAND}}-foundation-core")
...
if not result["valid"]:
 ...
 # Auto-split
 validator.auto_split_skill(".claude/skills/{{slot:BRAND}}-foundation-core")
...
# Usage
loader = ProgressiveContentLoader(".claude/skills/{{slot:BRAND}}-foundation-core")
...
Skills:
- {{slot:BRAND}}-foundation-modular-system - File organization patterns
- {{slot:BRAND}}-foundation-token-optimization - Content efficiency
- {{slot:BRAND}}-cc-skill-factory - Skill creation with progressive structure
...
Commands:
- /{{slot:BRAND_CMD}}:1-plan - Generate SPEC with progressive detail
- /{{slot:BRAND_CMD}}:3-sync - Create docs with layered structure
