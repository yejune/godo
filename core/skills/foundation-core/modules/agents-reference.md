...
|-------|---------|---------|
| `workflow-project` | `/{{slot:BRAND_CMD}}:0-project` | Project initialization and setup |
| `workflow-spec` | `/{{slot:BRAND_CMD}}:1-plan` | EARS SPEC generation and planning |
| `workflow-ddd` | `/{{slot:BRAND_CMD}}:2-run` | DDD ANALYZE-PRESERVE-IMPROVE execution |
| `workflow-docs` | `/{{slot:BRAND_CMD}}:3-sync` | Documentation generation and synchronization |
...
|-------|---------|---------|
| `core-planner` | `/{{slot:BRAND_CMD}}:2-run` Phase 1 | SPEC analysis and execution strategy |
...
|----------|--------|---------|
| Language (Separated) | {{slot:BRAND}}-lang-python, {{slot:BRAND}}-lang-typescript, {{slot:BRAND}}-lang-systems, {{slot:BRAND}}-lang-jvm, {{slot:BRAND}}-lang-mobile | Domain-specific language skills for 40-60% token savings |
| Platform (Separated) | {{slot:BRAND}}-platform-auth, {{slot:BRAND}}-platform-database, {{slot:BRAND}}-platform-deploy | Domain-specific platform skills for 30-50% token savings |
| Foundation | {{slot:BRAND}}-foundation-core, {{slot:BRAND}}-foundation-claude, {{slot:BRAND}}-foundation-context, {{slot:BRAND}}-foundation-quality | Core principles and quality gates |
| Workflow | {{slot:BRAND}}-workflow-spec, {{slot:BRAND}}-workflow-project, {{slot:BRAND}}-workflow-testing, {{slot:BRAND}}-workflow-jit-docs | Workflow automation and testing |
| Domain | {{slot:BRAND}}-domain-backend, {{slot:BRAND}}-domain-frontend, {{slot:BRAND}}-domain-database, {{slot:BRAND}}-domain-uiux | Domain expertise patterns |

Language Skills Selection Guide:

| Language Skill | Coverage | Use When |
|----------------|----------|----------|
| {{slot:BRAND}}-lang-python | Python 3.13, FastAPI, Django, pytest | Backend APIs, data science, automation |
| {{slot:BRAND}}-lang-typescript | TypeScript 5.9, React 19, Next.js 16, tRPC | Frontend, full-stack web development |
| {{slot:BRAND}}-lang-systems | Go 1.23, Rust 1.91, Fiber, Axum | Microservices, CLI tools, systems programming |
| {{slot:BRAND}}-lang-jvm | Java 21, Kotlin 2.0, Scala 3.4, Spring | Enterprise applications, big data |
| {{slot:BRAND}}-lang-mobile | Swift 6, Kotlin Android, Flutter 3.24 | iOS, Android, cross-platform mobile |

Platform Skills Selection Guide:

| Platform Skill | Providers | Use When |
|----------------|-----------|----------|
| {{slot:BRAND}}-platform-auth | Auth0, Clerk, Firebase Auth | Authentication implementation |
| {{slot:BRAND}}-platform-database | Supabase, Neon, Convex, Firestore | Database platform integration |
| {{slot:BRAND}}-platform-deploy | Vercel, Railway | Deployment and CI/CD |

Note: Agents now use specific skills based on their domain. Cross-language agents include {{slot:BRAND}}-lang-python and {{slot:BRAND}}-lang-typescript by default.
...
Skills:
- [{{slot:BRAND}}-foundation-core](../SKILL.md) - Parent skill (this module is part of it)
- [{{slot:BRAND}}-foundation-context](../../{{slot:BRAND}}-foundation-context/SKILL.md) - Token budget and session state
- [{{slot:BRAND}}-foundation-claude](../../{{slot:BRAND}}-foundation-claude/SKILL.md) - Claude Code configuration
...
Commands:
- `/{{slot:BRAND_CMD}}:0-project` → `workflow-project`
- `/{{slot:BRAND_CMD}}:1-plan` → `workflow-spec`
- `/{{slot:BRAND_CMD}}:2-run` → `workflow-ddd`
- `/{{slot:BRAND_CMD}}:3-sync` → `workflow-docs`
