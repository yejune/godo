...
decision = splitter.should_split(".claude/skills/{{slot:BRAND}}-foundation-core/SKILL.md")
...
if decision["split_needed"]:
 ...
 ".claude/skills/{{slot:BRAND}}-foundation-core",
...
# Usage
organizer = SkillOrganizer(".claude/skills/{{slot:BRAND}}-foundation-core")
...
with open(".claude/skills/{{slot:BRAND}}-foundation-core/NAVIGATION.md", 'w') as f:
 f.write(navigation)
```
...
# Usage
discovery = ModuleDiscovery(".claude/skills/{{slot:BRAND}}-foundation-core")
...
Skills:
- {{slot:BRAND}}-foundation-progressive-disclosure - Content structuring
- {{slot:BRAND}}-cc-skill-factory - Skill creation with modular structure
- {{slot:BRAND}}-foundation-token-optimization - File loading efficiency
...
Commands:
- /{{slot:BRAND_CMD}}:1-plan - SPEC generation with modular docs
- /{{slot:BRAND_CMD}}:3-sync - Documentation sync to modular structure

Memory:
- @.claude/skills/ - Standard skill location
- Skill("{{slot:BRAND}}-foundation-core") modules/ - Memory files following modular pattern
