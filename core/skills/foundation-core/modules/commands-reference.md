...
| ------------------ | ---------------------- | ------------- |
| `/{{slot:BRAND_CMD}}:0-project`  | Project initialization | Setup         |
| `/{{slot:BRAND_CMD}}:1-plan`     | SPEC generation        | Planning      |
| `/{{slot:BRAND_CMD}}:2-run`      | DDD implementation     | Development   |
| `/{{slot:BRAND_CMD}}:3-sync`     | Documentation sync     | Documentation |
| `/{{slot:BRAND_CMD}}:9-feedback` | Feedback collection    | Improvement   |
| `/{{slot:BRAND_CMD}}:99-release` | Production deployment  | Release       |

Required Workflow:
```
1. /{{slot:BRAND_CMD}}:0-project # Initialize
2. /{{slot:BRAND_CMD}}:1-plan "description" # Generate SPEC
3. /clear # Clear context (REQUIRED)
4. /{{slot:BRAND_CMD}}:2-run SPEC-001 # Implement
5. /{{slot:BRAND_CMD}}:3-sync SPEC-001 # Document
6. /{{slot:BRAND_CMD}}:9-feedback # Improve
```

Critical Rule: Execute `/clear` after `/{{slot:BRAND_CMD}}:1-plan` (saves 45-50K tokens)

---

## Implementation Guide (5 minutes)

### `/{{slot:BRAND_CMD}}:0-project` - Project Initialization
...
Usage:
```bash
/{{slot:BRAND_CMD}}:0-project
/{{slot:BRAND_CMD}}:0-project --with-git
```

What It Does:
1. Creates `.{{slot:BRAND_DIR}}/` directory structure
...
Output:
- `.{{slot:BRAND_DIR}}/` directory
- `.{{slot:BRAND_DIR}}/config/config.yaml`
- `.{{slot:BRAND_DIR}}/memory/` (empty, ready for session state)
- `.{{slot:BRAND_DIR}}/logs/` (empty, ready for logging)

Next Step: Ready for SPEC generation via `/{{slot:BRAND_CMD}}:1-plan`

Example:
```
User: /{{slot:BRAND_CMD}}:0-project
MoAI: Project initialized successfully.
 - .{{slot:BRAND_DIR}}/config/config.yaml created
...
### `/{{slot:BRAND_CMD}}:1-plan` - SPEC Generation
...
Usage:
```bash
/{{slot:BRAND_CMD}}:1-plan "Implement user authentication endpoint (JWT)"
/{{slot:BRAND_CMD}}:1-plan "Add dark mode toggle to settings page"
```

What It Does:
1. Analyzes user request
2. Generates EARS format SPEC document
3. Creates `.{{slot:BRAND_DIR}}/specs/SPEC-XXX/` directory
...
Output:
- `.{{slot:BRAND_DIR}}/specs/SPEC-001/spec.md` (EARS document)
...
Example:
```
User: /{{slot:BRAND_CMD}}:1-plan "Implement user authentication endpoint (JWT)"
MoAI: SPEC-001 generated successfully.
 Location: .{{slot:BRAND_DIR}}/specs/SPEC-001/spec.md
...
### `/{{slot:BRAND_CMD}}:2-run` - DDD Implementation
...
Usage:
```bash
/{{slot:BRAND_CMD}}:2-run SPEC-001
/{{slot:BRAND_CMD}}:2-run SPEC-002
```
...
Example:
```
User: /{{slot:BRAND_CMD}}:2-run SPEC-001
...
### `/{{slot:BRAND_CMD}}:3-sync` - Documentation Synchronization
...
Usage:
```bash
/{{slot:BRAND_CMD}}:3-sync SPEC-001
/{{slot:BRAND_CMD}}:3-sync SPEC-002
```
...
- `.{{slot:BRAND_DIR}}/docs/SPEC-001/` directory
- Project report

Example:
```
User: /{{slot:BRAND_CMD}}:3-sync SPEC-001
MoAI: Documentation synchronized for SPEC-001.

 Generated:
 - API documentation: .{{slot:BRAND_DIR}}/docs/SPEC-001/api.yaml
 - Architecture diagram: .{{slot:BRAND_DIR}}/docs/SPEC-001/architecture.md
 - Completion report: .{{slot:BRAND_DIR}}/docs/SPEC-001/report.md
```

---

### `/{{slot:BRAND_CMD}}:9-feedback` - Improvement Feedback Collection
...
Usage:
```bash
/{{slot:BRAND_CMD}}:9-feedback
/{{slot:BRAND_CMD}}:9-feedback --analyze SPEC-001
```
...
Example:
```
User: /{{slot:BRAND_CMD}}:9-feedback
...
### `/{{slot:BRAND_CMD}}:99-release` - Production Deployment
...
Rule 1: Execute `/clear` AFTER `/{{slot:BRAND_CMD}}:1-plan` (mandatory)
...
| ------------------ | ------------------ | ----------------------- |
| `/{{slot:BRAND_CMD}}:0-project`  | `workflow-project` | Tier 1 (Always Active)  |
| `/{{slot:BRAND_CMD}}:1-plan`     | `workflow-spec`    | Tier 1 (Always Active)  |
| `/{{slot:BRAND_CMD}}:2-run`      | `workflow-ddd`     | Tier 1 (Always Active)  |
| `/{{slot:BRAND_CMD}}:3-sync`     | `workflow-docs`    | Tier 1 (Always Active)  |
| `/{{slot:BRAND_CMD}}:9-feedback` | `core-quality`     | Tier 2 (Auto-triggered) |
| `/{{slot:BRAND_CMD}}:99-release` | `infra-devops`     | Tier 3 (Lazy-loaded)    |
...
| -------------- | -------------- | ------------------------------------- |
| `/{{slot:BRAND_CMD}}:1-plan` | 45-50K         | Planning Phase (30K allocated)        |
| `/{{slot:BRAND_CMD}}:2-run`  | 80-100K        | Implementation Phase (180K allocated) |
| `/{{slot:BRAND_CMD}}:3-sync` | 20-25K         | Documentation Phase (40K allocated)   |
| Total          | 145-175K       | 250K per feature                      |

Optimization:
- Use Haiku 4.5 for `/{{slot:BRAND_CMD}}:2-run` (fast, cost-effective)
- Use Sonnet 4.5 for `/{{slot:BRAND_CMD}}:1-plan` (high-quality SPEC)
...
| ------------------------- | ---------------------- | ------------------------------------------- |
| "Project not initialized" | `/{{slot:BRAND_CMD}}:1-plan`         | Run `/{{slot:BRAND_CMD}}:0-project` first                 |
| "SPEC not found"          | `/{{slot:BRAND_CMD}}:2-run SPEC-999` | Verify SPEC ID exists                       |
| "Token limit exceeded"    | Any                    | Execute `/clear` immediately                |
| "Test coverage < 85%"     | `/{{slot:BRAND_CMD}}:2-run`          | `core-quality` auto-generates missing tests |

Recovery Pattern:
```bash
# Error: Token limit exceeded
1. /clear # Reset context
2. /{{slot:BRAND_CMD}}:2-run SPEC-001 # Retry with clean context
```
...
```
/{{slot:BRAND_CMD}}:0-project → /{{slot:BRAND_CMD}}:1-plan → /clear → /{{slot:BRAND_CMD}}:2-run → /{{slot:BRAND_CMD}}:3-sync
```

Quick Workflow (No SPEC for simple tasks):
```
/{{slot:BRAND_CMD}}:0-project → Direct implementation (for 1-2 file changes)
```

Iterative Workflow (Multiple SPECs):
```
/{{slot:BRAND_CMD}}:1-plan "Feature A" → /clear → /{{slot:BRAND_CMD}}:2-run SPEC-001 → /{{slot:BRAND_CMD}}:3-sync SPEC-001
/{{slot:BRAND_CMD}}:1-plan "Feature B" → /clear → /{{slot:BRAND_CMD}}:2-run SPEC-002 → /{{slot:BRAND_CMD}}:3-sync SPEC-002
```
...
- `/{{slot:BRAND_CMD}}:1-plan`: Prompts for branch creation
- `/{{slot:BRAND_CMD}}:2-run`: Auto-commits to local branch
- No auto-push

Personal Mode (GitHub Individual):
- `/{{slot:BRAND_CMD}}:1-plan`: Auto-creates feature branch + auto-push
- `/{{slot:BRAND_CMD}}:2-run`: Auto-commits + auto-push
- `/{{slot:BRAND_CMD}}:3-sync`: Suggests PR creation (user choice)

Team Mode (GitHub Team):
- `/{{slot:BRAND_CMD}}:1-plan`: Auto-creates feature branch + Draft PR
- `/{{slot:BRAND_CMD}}:2-run`: Auto-commits + auto-push
- `/{{slot:BRAND_CMD}}:3-sync`: Prepares PR for team review
...
Skills:
- [{{slot:BRAND}}-foundation-core](../SKILL.md) - Parent skill
- [{{slot:BRAND}}-foundation-context](../../{{slot:BRAND}}-foundation-context/SKILL.md) - Token budget management
...
Agents:
- [workflow-project](agents-reference.md#tier-1-command-processors) - `/{{slot:BRAND_CMD}}:0-project`
- [workflow-spec](agents-reference.md#tier-1-command-processors) - `/{{slot:BRAND_CMD}}:1-plan`
- [workflow-ddd](agents-reference.md#tier-1-command-processors) - `/{{slot:BRAND_CMD}}:2-run`
- [workflow-docs](agents-reference.md#tier-1-command-processors) - `/{{slot:BRAND_CMD}}:3-sync`
