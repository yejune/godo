...
metadata:
  ...
  related-skills: "{{slot:BRAND}}-workflow-testing, {{slot:BRAND}}-foundation-quality, {{slot:BRAND}}-domain-backend, {{slot:BRAND}}-domain-frontend"
  context: "fork"
  agent: "Explore"
...
To scan with MoAI-ADK rules, execute sg scan with config pointing to .claude/skills/{{slot:BRAND}}-tool-ast-grep/rules/sgconfig.yml.
...
- {{slot:BRAND}}-workflow-testing: DDD integration and test pattern detection
- {{slot:BRAND}}-foundation-quality: TRUST 5 compliance and code quality gates
- {{slot:BRAND}}-domain-backend: API pattern detection and security scanning
- {{slot:BRAND}}-domain-frontend: React/Vue pattern optimization
- {{slot:BRAND}}-lang-python: Python-specific security and style rules
- {{slot:BRAND}}-lang-typescript: TypeScript type safety patterns
