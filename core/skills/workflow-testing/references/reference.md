# {{slot:BRAND}}-workflow-testing Reference
...
```bash
# CLI diagnostics
{{slot:BRAND}}-workflow diagnose --full-check
{{slot:BRAND}}-workflow test --dry-run --verbose
{{slot:BRAND}}-workflow profile --list-available
{{slot:BRAND}}-workflow review --show-rules
...
- Workflow logs: `.{{slot:BRAND_DIR}}/logs/workflow.log`
- Test results: `.{{slot:BRAND_DIR}}/test-results/`
- Profile data: `.{{slot:BRAND_DIR}}/profiles/`
- Review reports: `.{{slot:BRAND_DIR}}/reviews/`
- Debug sessions: `.{{slot:BRAND_DIR}}/debug-sessions/`
...
- {{slot:BRAND}}-foundation-core - TRUST 5 framework and SPEC-First TDD
- {{slot:BRAND}}-domain-backend - Backend development workflows
- {{slot:BRAND}}-domain-frontend - Frontend development workflows
- {{slot:BRAND}}-platform-baas - Backend-as-a-Service integration
