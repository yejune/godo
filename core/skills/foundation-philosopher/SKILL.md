---
name: {{slot:BRAND}}-foundation-philosopher
description: >
  ...
  Do NOT use for code quality validation (use {{slot:BRAND}}-foundation-quality instead)
  or implementation workflows (use {{slot:BRAND}}-workflow-ddd instead).
license: Apache-2.0
...
- Apply Assumption Audit during /{{slot:BRAND_CMD}}:1-plan
...
Skills:
- {{slot:BRAND}}-foundation-core: Integration with TRUST 5 and SPEC workflow
- {{slot:BRAND}}-workflow-spec: Assumption documentation in SPEC format
- {{slot:BRAND}}-domain-backend: Technology-specific trade-off criteria
- {{slot:BRAND}}-domain-frontend: UI/UX decision frameworks

Commands:
- /{{slot:BRAND_CMD}}:1-plan: Apply Philosopher Framework during specification
- /{{slot:BRAND_CMD}}:2-run: Reference documented trade-offs during implementation
