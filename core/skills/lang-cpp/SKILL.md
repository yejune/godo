...
- `{{slot:BRAND}}-lang-rust` - Systems programming comparison and interop
- `{{slot:BRAND}}-domain-backend` - Backend service architecture
- `{{slot:BRAND}}-workflow-testing` - DDD and testing strategies
- `{{slot:BRAND}}-essentials-debug` - Debugging and profiling
- `{{slot:BRAND}}-foundation-quality` - TRUST 5 quality principles
