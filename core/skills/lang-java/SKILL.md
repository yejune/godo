...
metadata:
  ...
  related-skills: "{{slot:BRAND}}-lang-kotlin, {{slot:BRAND}}-domain-backend"
...
- {{slot:BRAND}}-lang-kotlin for Kotlin interoperability and Spring Kotlin extensions
- {{slot:BRAND}}-domain-backend for REST API, GraphQL, and microservices architecture
- {{slot:BRAND}}-domain-database for JPA, Hibernate, and R2DBC patterns
- {{slot:BRAND}}-foundation-quality for JUnit 5, Mockito, and TestContainers integration
- {{slot:BRAND}}-infra-docker for JVM container optimization
