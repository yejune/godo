# {{slot:BRAND}}-workflow-project Reference
...
```python
# Integration with /{{slot:BRAND_CMD}}:1-plan and /{{slot:BRAND_CMD}}:3-sync
from moai_workflow_project import MoaiMenuProject

# Initialize project system
project = MoaiMenuProject("/path/to/project")

# After /{{slot:BRAND_CMD}}:1-plan generates SPEC
...
- Project logs: `.{{slot:BRAND_DIR}}/logs/project.log`
- Template optimization logs: `.{{slot:BRAND_DIR}}/logs/template-optimizer.log`
- Language initialization logs: `.{{slot:BRAND_DIR}}/logs/language-init.log`
...
- MoAI-ADK Documentation: https://github.com/{{slot:BRAND}}-adk/docs
- Claude Code Skills Guide: https://docs.anthropic.com/claude-code/skills
- SPEC-First DDD Methodology: See `{{slot:BRAND}}-foundation-core/modules/spec-first-ddd.md`

### Related Skills

- {{slot:BRAND}}-foundation-core - Core execution patterns and SPEC workflow
- {{slot:BRAND}}-foundation-claude - Claude Code integration patterns
- {{slot:BRAND}}-workflow-docs - Unified documentation management
- {{slot:BRAND}}-workflow-templates - Template optimization strategies
- {{slot:BRAND}}-library-nextra - Advanced documentation architecture
