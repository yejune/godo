# {{slot:BRAND}}-workflow-templates Reference
...
```python
# Integration with /{{slot:BRAND_CMD}}:9-feedback command
...
- Solution: Review conflicts in `.{{slot:BRAND_DIR}}/merge-conflicts/` and resolve manually
...
- Template storage: `.claude/skills/{{slot:BRAND}}-workflow-templates/templates/`
- Backups: `.{{slot:BRAND_DIR}}/backups/templates/`
- Merge conflicts: `.{{slot:BRAND_DIR}}/merge-conflicts/`
- Version history: `.{{slot:BRAND_DIR}}/config/template-versions.json`
- Feedback templates: `.claude/skills/{{slot:BRAND}}-workflow-templates/modules/feedback-templates.md`
...
- {{slot:BRAND}}-workflow-project - Project initialization with templates
- {{slot:BRAND}}-foundation-core - SPEC-driven template generation
