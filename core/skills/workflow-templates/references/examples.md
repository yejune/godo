...
**Scenario**: /{{slot:BRAND_CMD}}:9-feedback 명령으로 버그 리포트 이슈 생성

**Input**:
```bash
# 피드백 명령 실행
/{{slot:BRAND_CMD}}:9-feedback "로그인 시 JWT 토큰이 만료되어도 갱신되지 않음"
```
...
```json
{
  ...
  "user_customizations_preserved": [
    {
      "file": ".{{slot:BRAND_DIR}}/config/config.yaml",
      "preserved_fields": [
        "user.name",
  ...
  "conflicts_resolved": [
    {
      "file": ".{{slot:BRAND_DIR}}/config/sections/quality.yaml",
      "resolution": "Merged both changes",
      "details": "User's custom thresholds + new quality metrics"
    }
  ],
  "manual_review_required": [],
  "restoration_point": ".{{slot:BRAND_DIR}}/backups/pre-merge-2025-12-06/"
}
```
...
```json
// .{{slot:BRAND_DIR}}/config/template-history.json
{
...
```bash
# 잘못된 예시 - 템플릿 직접 수정
vim .claude/skills/{{slot:BRAND}}-workflow-templates/modules/code-templates.md
# → MoAI-ADK 업데이트 시 충돌
# → 팀원 간 불일치
```
...
# → .{{slot:BRAND_DIR}}/config/template-overrides.json에 저장
# → 템플릿 업데이트와 독립적
```
...
```
프로젝트 초기화:
/{{slot:BRAND_CMD}}:0-project → 코드 템플릿 선택 → 스캐폴딩 → 버전 추적

피드백 제출:
/{{slot:BRAND_CMD}}:9-feedback → 자동 분류 → 템플릿 적용 → GitHub 이슈 생성
