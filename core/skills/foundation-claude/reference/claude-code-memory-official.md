...
- /{{slot:BRAND_CMD}}:0-project: Initialize project structure
- /{{slot:BRAND_CMD}}:1-plan: Generate SPEC document
- /{{slot:BRAND_CMD}}:2-run: Execute DDD implementation
- /{{slot:BRAND_CMD}}:3-sync: Generate documentation
- /{{slot:BRAND_CMD}}:9-feedback: Collect improvement feedback

## Command Execution Rules

- After /{{slot:BRAND_CMD}}:1-plan: Execute /clear (mandatory)
- Token threshold: Execute /clear at >150K tokens
- Error handling: Use /{{slot:BRAND_CMD}}:9-feedback for all issues
```
...
# - .{{slot:BRAND_DIR}}/memory/ directory
# - CLAUDE.md template
