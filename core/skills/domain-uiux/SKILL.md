...
description: >
  ...
  (use {{slot:BRAND}}-domain-frontend instead) or shadcn/ui specifics
  (use {{slot:BRAND}}-library-shadcn instead).
license: Apache-2.0
...
- {{slot:BRAND}}-lang-typescript - TypeScript and JavaScript best practices
- {{slot:BRAND}}-foundation-core - TRUST 5 quality validation
- {{slot:BRAND}}-library-nextra - Documentation generation
- {{slot:BRAND}}-library-shadcn - shadcn/ui specialized patterns
...
- /{{slot:BRAND_CMD}}:2-run - DDD implementation cycle
- /{{slot:BRAND_CMD}}:3-sync - Documentation generation
