...
Purpose: Integration patterns for {{slot:BRAND}}-worktree with development tools, IDEs, terminals, CI/CD pipelines, and monitoring systems.
...
- Run Tests: Execute /{{slot:BRAND_CMD}}:2-run for the SPEC
- Sync Worktree: Run {{slot:BRAND}}-worktree sync command
...
- Command option completion for all {{slot:BRAND}}-worktree subcommands
...
Navigation Aliases:
- mw: Short alias for {{slot:BRAND}}-worktree
- mwl: List worktrees
...
- mwdev: Switch to worktree and start development with /{{slot:BRAND_CMD}}:2-run
