...
Purpose: Overview of integration patterns for {{slot:BRAND}}-worktree with MoAI-ADK workflow, development tools, and external systems.
...
Integration Points:
- MoAI-ADK Workflow: Seamless integration with /{{slot:BRAND_CMD}}:1-plan, /{{slot:BRAND_CMD}}:2-run, /{{slot:BRAND_CMD}}:3-sync
...
The basic integration workflow follows three phases. During the Plan Phase, the /{{slot:BRAND_CMD}}:1-plan command auto-creates a worktree for the SPEC. During the Development Phase, use the {{slot:BRAND}}-worktree go command to navigate to the isolated environment and run /{{slot:BRAND_CMD}}:2-run for DDD implementation. During the Sync Phase, use {{slot:BRAND}}-worktree sync to update the worktree and /{{slot:BRAND_CMD}}:3-sync for documentation synchronization.
...
MoAI-ADK Integration: Refer to {{slot:BRAND}}-adk-integration.md
- Plan Phase integration with /{{slot:BRAND_CMD}}:1-plan
- Development Phase integration with /{{slot:BRAND_CMD}}:2-run
- Sync Phase integration with /{{slot:BRAND_CMD}}:3-sync
...
Plan Phase (/{{slot:BRAND_CMD}}:1-plan):
...
Development Phase (/{{slot:BRAND_CMD}}:2-run):
...
Sync Phase (/{{slot:BRAND_CMD}}:3-sync):
...
Detailed Reference: Refer to {{slot:BRAND}}-adk-integration.md for complete workflow patterns
...
For automatic SPEC worktree creation, integrate with /{{slot:BRAND_CMD}}:1-plan by configuring auto-creation in worktree settings. Refer to {{slot:BRAND}}-adk-integration.md for implementation details.
...
For team collaboration with shared registries, configure team registry settings with developer-specific worktree prefixes. Refer to {{slot:BRAND}}-adk-integration.md for coordination patterns.

---

## Sub-Module References

MoAI-ADK Integration ({{slot:BRAND}}-adk-integration.md):
- Complete /{{slot:BRAND_CMD}}:1-plan integration patterns
- DDD-aware /{{slot:BRAND_CMD}}:2-run integration
- Sync Phase automation with /{{slot:BRAND_CMD}}:3-sync
