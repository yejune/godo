...
To set up parallel development, create worktrees for each SPEC using {{slot:BRAND}}-worktree new with the SPEC ID and description. For parallel execution, navigate to each worktree using {{slot:BRAND}}-worktree go and run /{{slot:BRAND_CMD}}:2-run in separate terminals or background processes. For integration, use {{slot:BRAND}}-worktree sync --all to synchronize all worktrees, then clean up merged worktrees with {{slot:BRAND}}-worktree clean --merged-only.
...
Worktree Root ({repo}/.{{slot:BRAND_DIR}}/worktrees/{project-name}/):
