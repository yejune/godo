...
Quick Diagnostics:
- Check worktree status: {{slot:BRAND}}-worktree status --all
- Verify registry integrity: {{slot:BRAND}}-worktree registry validate
...
Prevention:
- Always use {{slot:BRAND}}-worktree remove instead of manual directory deletion
...
Symptoms:
- {{slot:BRAND}}-worktree commands fail with JSON parsing errors
...
- Validate the rebuilt registry with {{slot:BRAND}}-worktree status --all

### Orphaned Registry Entries

Symptoms:
- Registry lists worktrees that no longer exist on disk
- {{slot:BRAND}}-worktree status shows worktrees as missing
...
Resolution Steps:
1. Run {{slot:BRAND}}-worktree status --all to identify orphaned entries
2. For each orphaned entry, confirm the directory truly does not exist
3. Remove orphaned entries from registry using {{slot:BRAND}}-worktree registry prune
...
- Use {{slot:BRAND}}-worktree commands for standard operations
...
Symptoms:
- /{{slot:BRAND_CMD}}:1-plan does not create expected worktree
- /{{slot:BRAND_CMD}}:2-run cannot find worktree for SPEC
...
Status Commands:
- {{slot:BRAND}}-worktree status --all: Shows all worktrees with sync status
- git worktree list: Native Git worktree listing
- {{slot:BRAND}}-worktree status SPEC-ID --detailed: Detailed status for specific worktree

Registry Commands:
- {{slot:BRAND}}-worktree registry validate: Checks registry integrity
- {{slot:BRAND}}-worktree registry prune: Removes orphaned entries
- {{slot:BRAND}}-worktree registry export: Exports registry for backup
...
Cleanup Commands:
- {{slot:BRAND}}-worktree clean --dry-run: Preview cleanup without changes
- {{slot:BRAND}}-worktree clean --merged-only: Clean only merged worktrees
- git worktree prune: Remove stale Git worktree metadata

Recovery Commands:
- {{slot:BRAND}}-worktree registry rebuild: Reconstruct registry from directories
- {{slot:BRAND}}-worktree remove SPEC-ID --keep-branch: Remove worktree, preserve branch
...
Weekly Tasks:
- Run {{slot:BRAND}}-worktree status --all to check for issues
...
Creation:
- Always use {{slot:BRAND}}-worktree new instead of manual Git commands
...
Removal:
- Use {{slot:BRAND}}-worktree remove instead of manual deletion
