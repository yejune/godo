...
Purpose: External resources, documentation, and additional learning materials for {{slot:BRAND}}-worktree skill.
...
- Click Framework: [https://click.palletsprojects.com/](https://click.palletsprojects.com/)
 - Command-line interface framework used by {{slot:BRAND}}-worktree
...
- Rich Library: [https://rich.readthedocs.io/](https://rich.readthedocs.io/)
 - Terminal output formatting used by {{slot:BRAND}}-worktree
...
- {{slot:BRAND}}-foundation-core: Foundation principles and delegation patterns
 - Reference: [{{slot:BRAND}}-foundation-core modules]({{slot:BRAND}}-foundation-core/modules/)
 - Integration: Worktree creation follows foundation delegation patterns

- {{slot:BRAND}}-workflow-project: Project management and configuration
 - Reference: [{{slot:BRAND}}-workflow-project modules]({{slot:BRAND}}-workflow-project/modules/)
 - Integration: Project setup with worktree support

- {{slot:BRAND}}-foundation-claude: Claude Code execution patterns
 - Reference: [{{slot:BRAND}}-foundation-claude modules]({{slot:BRAND}}-foundation-claude/modules/)
...
1. Shallow Worktrees: For fast prototyping and testing
 ```bash
 {{slot:BRAND}}-worktree new SPEC-PROTO-001 "Prototype" --shallow --depth 1
 ```

2. Selective Synchronization: Sync only essential files
 ```bash
 {{slot:BRAND}}-worktree sync SPEC-001 --include "src/" --exclude "node_modules/"
 ```

3. Background Operations: Non-blocking worktree operations
 ```bash
 {{slot:BRAND}}-worktree sync --all --background
 ```
...
jobs:
 ...
 - name: Setup {{slot:BRAND}}-worktree
 run: |
 go install github.com/modu-ai/{{slot:BRAND}}-adk-go/cmd/moai@latest
 echo "Setting up worktree environment..."

 - name: Test Worktree Operations
 run: |
 # Test worktree creation
 {{slot:BRAND}}-worktree new test-spec "Test Worktree"

 # Test worktree synchronization
 {{slot:BRAND}}-worktree sync test-spec

 # Test worktree cleanup
 {{slot:BRAND}}-worktree remove test-spec
```
...
pipeline {
 ...
 sh '''
 go install github.com/modu-ai/{{slot:BRAND}}-adk-go/cmd/moai@latest
 {{slot:BRAND}}-worktree config set worktree_root $WORKSPACE/worktrees
 '''
 }
 }
 ...
 steps {
 sh '''
 {{slot:BRAND}}-worktree new SPEC-AUTH-001 "Authentication Worktree"
 cd $({{slot:BRAND}}-worktree go SPEC-AUTH-001)
 npm test
 '''
 }
 }
 ...
 steps {
 sh '''
 {{slot:BRAND}}-worktree new SPEC-PAY-001 "Payment Worktree"
 cd $({{slot:BRAND}}-worktree go SPEC-PAY-001)
 npm test
 '''
 }
 }
 ...
 steps {
 sh '''
 {{slot:BRAND}}-worktree clean --force
...
git clone https://github.com/your-org/{{slot:BRAND}}-adk.git
...
Version: 1.0.0
Last Updated: 2025-11-29
Reference: External resources and additional learning materials for {{slot:BRAND}}-worktree
