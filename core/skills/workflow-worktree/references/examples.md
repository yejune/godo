...
Purpose: Real-world usage examples and patterns for {{slot:BRAND}}-worktree skill integration with MoAI-ADK workflow.
...
/{{slot:BRAND_CMD}}:1-plan "User Authentication System" --worktree

# Output:
# SPEC created: SPEC-AUTH-001
# Worktree created: .{{slot:BRAND_DIR}}/worktrees/MoAI-ADK/SPEC-AUTH-001
#
# Next steps:
# 1. Switch to worktree: {{slot:BRAND}}-worktree switch SPEC-AUTH-001
# 2. Or use shell eval: eval $({{slot:BRAND}}-worktree go SPEC-AUTH-001)
# 3. Start development: /{{slot:BRAND_CMD}}:2-run SPEC-AUTH-001

# Development Phase - Implement in isolated environment
eval $({{slot:BRAND}}-worktree go SPEC-AUTH-001)
/{{slot:BRAND_CMD}}:2-run SPEC-AUTH-001

# Sync Phase - Synchronize and integrate
{{slot:BRAND}}-worktree sync SPEC-AUTH-001
/{{slot:BRAND_CMD}}:3-sync SPEC-AUTH-001

# Cleanup Phase - Remove completed worktree
{{slot:BRAND}}-worktree remove SPEC-AUTH-001
```
...
{{slot:BRAND}}-worktree new SPEC-AUTH-001 "User Authentication"
{{slot:BRAND}}-worktree new SPEC-PAY-001 "Payment Processing"
{{slot:BRAND}}-worktree new SPEC-DASH-001 "Dashboard Analytics"

# Switch between worktrees
{{slot:BRAND}}-worktree switch SPEC-AUTH-001
# Work on authentication...

{{slot:BRAND}}-worktree switch SPEC-PAY-001
# Work on payment system...

{{slot:BRAND}}-worktree switch SPEC-DASH-001
# Work on dashboard...

# Check status of all worktrees
{{slot:BRAND}}-worktree status --all

# Sync all worktrees
{{slot:BRAND}}-worktree sync --all

# Clean up completed worktrees
{{slot:BRAND}}-worktree clean --merged-only
```
...
```bash
# /{{slot:BRAND_CMD}}:2-run automatically detects worktree environment
/{{slot:BRAND_CMD}}:2-run SPEC-AUTH-001
...
{{slot:BRAND}}-worktree status SPEC-AUTH-001
{{slot:BRAND}}-worktree sync SPEC-AUTH-001 --include "src/"
{{slot:BRAND}}-worktree config get worktree_root
```
...
Skill("{{slot:BRAND}}-worktree") # Load worktree patterns

# Setup project with worktree support
project_config = {
 "name": "User Authentication System",
 "worktree_enabled": True,
 "worktree_root": ".{{slot:BRAND_DIR}}/worktrees/UserAuth",
...
/{{slot:BRAND_CMD}}:1-plan "$SPEC_DESCRIPTION" --worktree --spec-id "$SPEC_ID"

# Check if worktree was created successfully
if {{slot:BRAND}}-worktree list --format json | jq -r ".worktrees[\"$SPEC_ID\"]" > /dev/null; then
 echo " Worktree $SPEC_ID created successfully"

 # Phase 2: Develop
 echo " Phase 2: Switching to worktree for development..."
 cd $({{slot:BRAND}}-worktree go "$SPEC_ID")

 # Development loop
 while true; do
 echo " Running DDD implementation..."
 /{{slot:BRAND_CMD}}:2-run "$SPEC_ID"
 ...
 {{slot:BRAND}}-worktree sync "$SPEC_ID"
 cd - # Return to main repository
 /{{slot:BRAND_CMD}}:3-sync "$SPEC_ID"
 ...
 {{slot:BRAND}}-worktree remove "$SPEC_ID"
 echo " Worktree $SPEC_ID removed"
 fi

else
 echo " Worktree creation failed. Falling back to branch development."
 /{{slot:BRAND_CMD}}:1-plan "$SPEC_DESCRIPTION" --branch --spec-id "$SPEC_ID"
fi
...
WORKTREE_ROOT=".{{slot:BRAND_DIR}}/worktrees/$PROJECT_NAME"
...
{{slot:BRAND}}-worktree config set worktree_root "$WORKTREE_ROOT"
{{slot:BRAND}}-worktree config set registry_type team

echo " Team worktree configuration completed"
echo "Team members can now join with: {{slot:BRAND}}-worktree join --team $TEAM_NAME"
```
...
DEBUG=true""",
 ...
 # Save template
 template_path = Path.home() / ".{{slot:BRAND}}-worktree/templates" / "fullstack.json"
...
# {{slot:BRAND}}-worktree new SPEC-FULL-001 "Fullstack Application" --template fullstack
```
...
class WorktreeAutomation:
 ...
 "{{slot:BRAND}}-worktree", "new", spec_id, description,
 "--template", template
 ...
 result = subprocess.run([
 "{{slot:BRAND}}-worktree", "list", "--status", "active", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 "{{slot:BRAND}}-worktree", "status", "--all", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 'action': '{{slot:BRAND}}-worktree clean --stale --days 30'
 })
 ...
 'action': '{{slot:BRAND}}-worktree optimize --analyze'
 })
...
{{slot:BRAND}}-worktree new SPEC-DEBUG-001 "Debug Test" --verbose

# Problem 2: Worktree sync conflicts
echo " Resolving sync conflicts..."

# Check sync status
{{slot:BRAND}}-worktree status SPEC-CONFLICT-001

# Interactive conflict resolution
{{slot:BRAND}}-worktree sync SPEC-CONFLICT-001 --interactive

# Force sync (if appropriate)
{{slot:BRAND}}-worktree sync SPEC-CONFLICT-001 --force

# Problem 3: Worktree registry corruption
echo " Repairing worktree registry..."

# Backup current registry
cp .{{slot:BRAND_DIR}}/worktrees/PROJECT/.{{slot:BRAND}}-worktree-registry.json .{{slot:BRAND_DIR}}/worktrees/PROJECT/.{{slot:BRAND}}-worktree-registry.json.backup

# Rebuild registry from worktree directories
{{slot:BRAND}}-worktree config set registry_rebuild true
{{slot:BRAND}}-worktree list --rebuild-registry

# Problem 4: Permission issues
echo " Fixing permission issues..."

# Check worktree permissions
ls -la .{{slot:BRAND_DIR}}/worktrees/PROJECT/

# Fix permissions
chmod -R 755 .{{slot:BRAND_DIR}}/worktrees/PROJECT/
```
...
Version: 1.0.0
Last Updated: 2025-11-29
Examples: Real-world usage patterns for {{slot:BRAND}}-worktree integration
