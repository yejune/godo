...
**MoAI**: 에이전트 실행 사이클에 대한 공식 프로토콜이 없다. 에이전트는 작업을 수행하고 결과를 반환한다.

**Do**: 모든 에이전트는 6단계 멱등 실행 사이클을 따른다:

| 단계 | 행동 | 멱등성 보장 |
|------|------|-----------|
| **READ** | 체크리스트 읽기 -- 현재 상태 파악 | 항상 최신 상태에서 시작 |
| **CLAIM** | 미완료 항목을 `[~]`로 전환 | 중복 작업 방지 |
| **WORK** | 코드 작성 + 테스트 | 실제 구현 |
| **VERIFY** | 테스트 통과 확인 | 품질 보장 |
| **RECORD** | 체크리스트 상태 갱신 (`[o]` + commit 해시) | 영속 상태 기록 |
| **COMMIT** | 코드 + 체크리스트 함께 커밋 | 원자적 완료 증명 |

핵심 규칙:
- **체크리스트 미갱신 = VIOLATION**: 코드만 커밋하고 체크리스트를 갱신하지 않으면 위반이다.
- **코드 + 체크리스트 함께 커밋**: 코드 변경과 체크리스트 상태 변경이 동일 커밋에 포함된다.
- **어떤 에이전트든 이어받기 가능**: READ 단계에서 체크리스트를 읽으면 정확히 어디서부터 재개해야 하는지 알 수 있다.

이 사이클은 Do의 체크리스트-as-state-file 철학의 완성이다. 체크리스트가 단순한 할일 목록이 아니라 에이전트의 **트랜잭션 로그**로 기능한다.
...
체크리스트는 단순한 작업 목록이 아니라 **파일 소유권의 경계선**이기도 하다.

#### 원칙: 한 파일 = 한 소유자

팀 모드에서 에이전트를 배정할 때, 각 서브 체크리스트의 Critical Files 섹션이 해당 에이전트의 **파일 소유권 범위**를 정의한다. 같은 파일을 두 에이전트가 동시에 수정하는 것은 원천적으로 차단된다.

| 상황 | 처리 방식 |
|------|----------|
| 파일 A를 에이전트 1만 수정 | 정상 — 소유권 충돌 없음 |
| 파일 A를 에이전트 1과 2가 동시에 수정 필요 | **금지** — 선행 에이전트 완료 후 후행 에이전트 진행 (`depends on:`) |
| 에이전트 2가 에이전트 1의 파일을 참조만 함 | 허용 — 참조는 소유권 침해가 아님 |

#### `git add -A` / `git add .` 금지

에이전트는 자신의 소유 파일만 스테이징해야 한다. `git add -A`나 `git add .`는 다른 에이전트의 작업 파일, 미완성 변경, 또는 민감 파일(.env, credentials)을 실수로 포함할 수 있다.

**필수 패턴:**
```bash
# 올바른 방법: 파일명을 명시적으로 지정
git add path/to/my-file.go path/to/my-file_test.go
...
이 규칙은 FINAL STEP: Commit 섹션의 `git add` 단계에서 강제된다. 서브 체크리스트의 Critical Files에 명시된 파일만 스테이징 대상이다.

#### 충돌 발생 시 프로토콜

만약 개발 중 예상치 못한 파일 수정이 필요해진 경우:
1. 해당 파일의 소유자를 메인 체크리스트에서 확인
2. 소유자가 작업 완료(`[o]`)되었으면 → 직접 수정 가능
3. 소유자가 작업 중(`[~]`)이면 → `[\!]` 블로커 선언, 소유자 완료 대기
4. 소유자가 없는 파일이면 → 메인 체크리스트에 소유권 등록 후 수정

이 구조가 Do의 팀 모드에서 **병렬 작업의 안전성을 보장**한다. MoAI의 Do 모드(Task tool 기반 순차/병렬)에는 파일 소유권 개념이 없어 충돌 가능성이 존재한다.
...
3회의 구조는 "같은 접근법 2회 + 다른 접근법 1회"이다:

| 시도 | 행동 | 근거 |
|------|------|------|
| 1회 실패 | 같은 방법 재시도 | 일시적 오류일 수 있음 (네트워크, 타이밍, 컨테이너 상태) |
| 2회 실패 | 접근법 자체를 재검토 | 같은 방법이 2번 실패하면 방법이 틀린 것. 무작정 반복은 토큰 낭비 |
| 3회 실패 | 에스컬레이션 | 재검토한 접근법도 실패 → 이 에이전트의 전문 영역 밖 |

핵심은 2회 실패가 **접근법 전환의 트리거**라는 것이다. 무작정 같은 방법을 3번 반복하는 것이 아니라, 2회 실패 시점에서 "왜 실패했는가?"를 분석하고 다른 접근법으로 전환한다. 전환해도 안 되면 전문 에이전트로 넘긴다.
...
**Do의 사용 방식**: Do는 체크리스트 작성 시점에 항목별로 개발 방법론을 결정한다. 모든 항목이 동일한 방법론을 따를 필요가 없다 — 어떤 항목은 TDD(테스트 먼저), 어떤 항목은 DDD(기존 코드 분석 먼저), 어떤 항목은 단순 구현으로 진행할 수 있다. AI가 각 항목의 성격을 판단하여 적합한 방법론을 제안하고, Test Strategy 섹션에 명시한다. 사용자에게 일괄 질문("TDD로 할까요?")하는 것이 아니라, 항목 단위로 최적의 접근법을 선택하는 것이다.
...
- **EARS**: 요구사항을 **어떻게 쓸 것인가** (문법/형식)
- **MoSCoW**: 요구사항을 **무엇부터 할 것인가** (우선순위)

| 축 | EARS | MoSCoW |
|-----|------|--------|
| 관심사 | 문장 구조 | 우선순위 분류 |
| 질문 | "이 요구사항을 어떤 형식으로 표현하나?" | "이 요구사항을 언제 구현하나?" |
| 강점 | 모호함 제거, 유형별 구조화 | 단순, 직관적, 반복 개발에 적합 |

**Do의 통합 방식**: EARS 형식으로 요구사항을 작성하고, MoSCoW로 우선순위를 분류한다.

예시:
| EARS 유형 | 요구사항 (EARS 형식) | MoSCoW |
|-----------|---------------------|--------|
| Event-driven | 로그인 실패 시, 시스템은 3초 대기 후 재시도를 허용한다 | **MUST** |
| State-driven | 오프라인 상태에서, 시스템은 로컬 캐시를 사용한다 | **SHOULD** |
| Unwanted | DB 연결 실패 시, 시스템은 자동 재연결을 3회 시도한다 | **MUST** |
| Optional | 알림 활성화 시, 시스템은 이메일을 발송한다 | **COULD** |
| Ubiquitous | 시스템은 모든 API 요청을 구조화된 로그로 기록한다 | **SHOULD** |

**Do의 철학 반영**: 
- analysis.md에서 요구사항을 EARS 형식으로 구조화 → 모호함 제거
- 각 요구사항에 MoSCoW 우선순위 부여 → 체크리스트 항목 순서 결정
...
| EARS + MoSCoW 통합 | **채택** | EARS 형식 + MoSCoW 우선순위를 결합하여 analysis.md에서 사용 |
| 디펜던시 타입 시스템 | **채택** | 6종 타입(phase/artifact/agent/env/service/checklist) + Hybrid 스키마 |
| 멱등 에이전트 실행 사이클 | **Do 고유** | READ->CLAIM->WORK->VERIFY->RECORD->COMMIT 6단계, 체크리스트 미갱신=VIOLATION |
| XML 완료 마커 | **거부** | commit 해시가 더 나은 증거 |
| 통합 진입점 | **거부** | 인텐트 라우팅보다 명시적 명령 |

### 10.11 채택: 디펜던시 타입 시스템 (6종)

**개요**: 체크리스트 항목 간 의존성을 유형별로 분류하는 시스템.

**채택 이유**: 단순한 `depends on:` 키워드만으로는 의존성의 성격을 구분할 수 없다. 왜 기다려야 하는지, 무엇이 해소되어야 하는지를 명시하면 블로커 해소가 빨라진다.

**6종 디펜던시 타입**:

| 타입 | 의미 | 예시 |
|------|------|------|
| **phase** | 워크플로우 단계 선후 관계 | Analysis 완료 후 Architecture |
| **artifact** | 산출물 의존 | architecture.md 완료 후 Plan |
| **agent** | 에이전트 작업 완료 대기 | backend 완료 후 frontend |
| **env** | 환경/인프라 준비 대기 | Docker 서비스 healthy 후 테스트 |
| **service** | 외부 서비스 의존 | OAuth 키 발급 후 소셜 로그인 |
| **checklist** | 체크리스트 항목 간 참조 | #1 완료 후 #2 시작 |

**Hybrid 스키마**: 간단한 의존성은 기존 `depends on: #1` 형식 유지, 복잡한 의존성은 타입을 명시: `depends on: #1 (type: agent, reason: API 인터페이스 필요)`.

**MoAI 비교**: MoAI의 TAG Chain은 작업 추적 메커니즘이지 의존성 유형 시스템이 아니다. Do의 디펜던시 타입은 체크리스트의 `[\!]` 블로커 상태와 직접 연동되어, 왜 블로킹되었는지를 기계 판독 가능한 형태로 기록한다.
...
**Progress Log 타임스탬프 규칙**: 체크리스트 Progress Log의 타임스탬프는 **초 단위 정밀도**(`YYYY-MM-DD HH:MM:SS`)를 사용한다. 에이전트의 작업 속도는 인간과 다르다 -- 분 단위(HH:MM)로는 여러 상태 전이가 동일 타임스탬프를 가질 수 있어 순서가 모호해진다. 초 단위가 이벤트 순서를 명확히 보장한다.

**날짜 기반 Jobs 디렉토리**: 산출물 경로가 `.do/jobs/{YY}/{MM}/{DD}/{title}/` nested 형식을 사용한다. 기존 flat 형식(`{YYMMDD}/`)은 폐지되었다.

**멱등 에이전트 실행 사이클**: 에이전트는 READ->CLAIM->WORK->VERIFY->RECORD->COMMIT 6단계를 따른다. 코드와 체크리스트를 함께 커밋하며, 체크리스트 미갱신은 VIOLATION이다.
...
### ADR-04: Progressive Disclosure 채택

**결정**: MoAI의 Progressive Disclosure(3단계 로딩) 채택. 규칙만 사용하는 기존 방식에서 스킬 기반 Progressive Disclosure로 전환
**MoAI 비교**: 6개 오버라이드 스킬 + 3단계 로딩 (메타데이터 → 본문 → 번들)
**근거**: 규칙(rules)은 항상 전체 로드되어 토큰을 소비한다. 에이전트가 많아질수록 불필요한 규칙까지 로드되는 비효율이 커진다. Progressive Disclosure는 필요한 지식만 필요한 시점에 로드하여 토큰을 절약한다. 또한 agent_patches를 통해 에이전트별 맞춤 지식을 주입할 수 있어 확장성이 뛰어나다.
**이전 결정**: 규칙만 사용 (단일 레이어 단순성). 에이전트 수가 적을 때는 충분했으나, 확장 시 토큰 낭비가 심해지는 한계가 확인됨.
...
**결정**: `.do/jobs/{YY}/{MM}/{DD}/{title-kebab-case}/` (nested 디렉토리)
**MoAI 비교**: `.moai/specs/SPEC-XXX/`
**이전 형식**: `.do/jobs/{YYMMDD}/{title}/` (flat) -- 폐지됨
**근거**: 날짜 기반 구성은 시간순 탐색을 가능하게 한다. 작업 폴더를 보면 언제 작업이 수행되었는지 즉시 알 수 있다. 번호 기반 SPEC-XXX는 순서를 이해하기 위해 레지스트리를 조회해야 한다. nested 형식(`YY/MM/DD/`)은 flat 형식(`YYMMDD/`)보다 파일 시스템 탐색이 직관적이며, 월별/일별 그룹핑이 자연스럽다.
...
| (없음) | 멱등 에이전트 실행 사이클 (6단계) | Do 고유 -- READ->CLAIM->WORK->VERIFY->RECORD->COMMIT |
| (없음) | 디펜던시 타입 시스템 (6종) | Do 고유 -- phase/artifact/agent/env/service/checklist |
| (없음) | DO_AI_FOOTER 환경변수 | Do 고유 -- 커밋 메시지 AI 푸터 제어 |
