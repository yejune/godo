...
- **Test Strategy**: 테스트 접근법 명시 (unit, integration, E2E, 또는 조합). 실제 테스트는 반드시 이 전략에 기반해야 한다. 가벼운 수정(CSS, 설정, 문서 등)의 경우 "pass"를 명시하여 테스트 생략 근거를 남긴다
- **Risks**: 깨질 수 있는 것
- **Progress Log**: 타임스탬프가 포함된 상태 변경 및 작업 이력
- **FINAL STEP: Commit**: git add, diff 확인, commit -- 절대 생략 금지
- **Lessons Learned**: 완료 시 필수 작성. 소회에서 개선사항이 발견되면 → 추가 서브 체크리스트 작성을 지시하고, 메인 체크리스트의 해당 항목을 `[~]`로 되돌린 후 개선 작업을 다시 진행한다

이 구조의 핵심은 **테스트 전략이 사후가 아닌 사전에 결정**된다는 것이다. 에이전트가 코드를 작성한 후에 "어떤 테스트를 할까?" 고민하는 게 아니라, 작업 시작 전에 "이 작업은 unit test로 검증한다" 또는 "이 작업은 빌드 확인으로 충분하다 (pass)"를 선언한다. 테스트 전략이 "pass"인 경우에도 그 판단 근거가 기록으로 남는다 — 이것이 추적성이다.

또한 Lessons Learned는 단순 회고가 아니라 **피드백 루프의 트리거**다. 소회에서 "이 부분의 에러 핸들링이 부족하다"는 발견이 나오면, 그것은 새로운 서브 체크리스트 항목이 되어 다시 개발 사이클을 탄다. 체크리스트가 살아있는 문서인 이유가 여기에 있다.
